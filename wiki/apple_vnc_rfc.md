...
Type-33 password preprocessing uses a two-stage derivation. The first stage applies PBKDF2 to harden the raw password against brute-force search; the second stage feeds the stretched material into standard SRP-6a as the password input.

**Stage 1 — PBKDF2 stretching:**

```text
P' = PBKDF2-HMAC-SHA512(password_utf8, salt, iterations, dkLen=128)
```

The output `P'` is 128 bytes. The iteration count is `156250`.

**Stage 2 — SRP-6a with stretched password:**

- `PAD(x)`: left-padding to the width of `N` in bytes
- `H`: SHA-512
- `OS2IP`: big-endian octet-string-to-integer conversion

The SRP identity is the **empty string**. This is a deliberate design choice: the username was already transmitted to the server and implicitly authenticated through RSA encryption in the packet-1 exchange (§4.2.4.3). Re-using the username in the SRP phase would be redundant; using the empty string as the identity avoids re-transmission while preserving the standard SRP-6a identity-binding structure.
...
A  = g^a mod N                          (client public value)

u  = OS2IP(H(PAD(A) || PAD(B)))         (shared blending parameter)

v  = g^x mod N                          (verifier; held by server, not sent on wire)

S  = (B - k*v mod N)^(a + u*x) mod N    (client-side shared secret)

K  = H(PAD(S))                          (hashed session secret)
...
- `B` is the server SRP public value from the challenge (§4.2.4.4)
- `N` and `g` are the SRP group parameters from the challenge
- `salt` is the salt from the challenge
- `K` is the hashed shared secret (the SRP session key)

**Server-side computation:**

The server holds the verifier `v` (computed during enrollment as `v = g^x mod N`) and generates a fresh secret exponent `b`. Upon receiving the client's `A`, the server computes:

```text
u  = OS2IP(H(PAD(A) || PAD(B)))
S  = (A * v^u)^b mod N                  (server-side shared secret)
K  = H(PAD(S))
M1 = H(H(PAD(N)) xor H(PAD(g)) || H("") || salt || PAD(A) || PAD(B) || K)   (expected client proof)
M2 = H(PAD(A) || M1 || K)               (server proof, sent as server_proof in §4.2.4.7)
```

The server can compute `K`, `M1`, and `M2` immediately upon receiving `A` because it possesses the verifier `v` and its own secret `b`. It stores the expected `M1` and the pre-computed `M2`, then sends `B` to the client. When the client's `M1` arrives in packet-2 (§4.2.4.5), the server compares it against the stored expected value; on match, it releases `M2` as the final proof.
...
The `server_proof` field carries the SRP-6a server evidence message `M2`, computed as:

```text
M2 = H(PAD(A) || M1 || K)
```

where `A` is the client's public value, `M1` is the client proof received in packet-2 (§4.2.4.5), and `K` is the hashed session secret from §4.2.4.6. Because the server holds the verifier `v`, it can pre-compute `K`, the expected `M1`, and `M2` immediately upon receiving `A` (§4.2.4.6); it verifies the client's `M1` on arrival and releases `M2` without additional computation.

The maintained final-proof payload grammar is:

```text
%o      server_proof      (M2)
%o      server_random     (fresh 16-byte random value)
%s      empty_string
%u      zero
```

The `server_random` field carries a fresh 16-byte random value generated by the server. It is not used in the SRP proof computation; it is carried alongside the proof and made available for later protocol use. The `empty_string` and `zero` fields are present for structure alignment and carry no semantic content in this revision.
...
**Architectural note — GSS proxy model.** The GSS-API operations on the server side are not performed directly by the VNC server process. A system-wide GSS daemon proxies context establishment and session-key extraction on behalf of the VNC server. The separate token exchange (AP-REQ, AP-REP, wrap token) reflects this proxied architecture: the daemon accepts the security context and returns the session key to the VNC server through either the wrap-token plaintext or the Lucid context export path described in §4.2.5.3. The server-side zero words that bracket the token exchange are application-level acknowledgements between the VNC server and the daemon, not GSS-API protocol elements.
...
2. Otherwise, export the GSS context as a Lucid Kerberos V5 context. The Lucid context carries a protocol version field that determines which subkey source is relevant:
   - **Protocol version 1 (CFX / RFC 4121):** if the acceptor subkey is present, use it; otherwise fall back to the CFX context key.
   - **Protocol version 0 (RFC 1964):** use the RFC 1964 context key.
   
   The first 16 bytes of the selected subkey form the session key. The full subkey is typically 32 bytes (for AES-256 Kerberos session keys) or longer; the 16-byte truncation selects the material needed for the AES-128 record layer (§6.4).
...
The cryptographic profile for this branch is:

| Parameter | Value |
|---|---|
| DH group | RFC 5054, 4096-bit MODP |
| Generator | `5` |
| Hash | SHA-512 |
| Password pre-processing | PBKDF2-HMAC-SHA512 (as indicated by `kdf=SALTED-SHA512-PBKDF2`) |
| Secure layer cipher | ChaCha20-Poly1305 (negotiated via `conf+int=`) |

ChaCha20-Poly1305 is the standard IETF AEAD construction specified in RFC 8439. An implementation MAY use any conformant ChaCha20-Poly1305 implementation (e.g. libsodium, OpenSSL 1.1+, BoringSSL). No Apple-specific modification to the cipher is known; the negotiation via the `conf+int` option string selects standard ChaCha20-Poly1305. The key and nonce derivation from the SRP session key `K` is a revision gap (§14).

The type-36 branch uses the same SRP-6a protocol as type 33 (§4.2.4.6) for the authentication exchange: the client sends `A`, the server responds with `B`, both derive `K`, and the client proves possession of the password-derived verifier through `M1` / `M2`. Unlike type 33, there is no RSA wrapping phase — the branch enters SRP directly after the initial branch-entry payload, and the negotiated secure layer (ChaCha20-Poly1305) provides both confidentiality and integrity for subsequent traffic, replacing the AES-CBC + SHA-1 record layer used by other branches (§6.4).
...
- exact key and nonce derivation for ChaCha20-Poly1305 from the type-36 SRP session key `K` (§4.2.6.2); the AEAD construction itself is standard (RFC 8439), but the key schedule that transforms `K` into the ChaCha20 key and the per-record nonce is not yet specified
