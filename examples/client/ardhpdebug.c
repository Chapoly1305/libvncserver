 ...
 *   VNC_ARD_HP_MEDIA_SRTP_KEY_HEX=<46-byte-hex>   (optional, adaptive suite-5 SRTP key candidate)
 *   VNC_ARD_HP_MEDIA_SRTP_KEY2_HEX=<46-byte-hex>  (optional, additional key candidate)
 *   VNC_ARD_HP_MEDIA_SRTP_KEY3_HEX=<46-byte-hex>  (optional, additional key candidate)
 *   VNC_ARD_HP_VIDEO_DECODER=auto|videotoolbox|ffmpeg (optional, default auto)
 *   VNC_ARD_HP_STRICT_STAGE2=1 (optional, deterministic stage1->0x1c->stage2 handshake)
 *   VNC_ARD_HP_DUMP_DIR=<dir>  (optional, dump decrypted packets/AUs for offline analysis)
 */
...
#if defined(ARDHPDEBUG_HAS_FFMPEG)
...
struct ard_hp_session_state {
  ...
  uint16_t media_stage1_udp_port;
  int media_udp_started;
  int adaptive_media_options_sent;
  int adaptive_delayed_setenc_pending;
  int adaptive_delayed_setenc_sent;
  unsigned int adaptive_post_1c_server_rects;
  int adaptive_followup_encodings_sent;
};
...
struct ard_hp_udp_media_state {
  int video_sock;
  int audio_sock;
  int enabled;
  int family;
  int video_source_socket;
  long long video_socket_substantial_ms;
  long long audio_socket_substantial_ms;
  uint16_t server_video_port;
  uint16_t server_audio_port;
  uint16_t local_video_port;
  uint16_t local_audio_port;
  char server_ip[64];
  unsigned long video_packets_rx;
  unsigned long audio_packets_rx;
  unsigned long video_decode_attempts;
  unsigned long video_decode_failures;
  size_t video_min_len;
  size_t video_max_len;
  size_t audio_min_len;
  size_t audio_max_len;
  long long last_stats_log_ms;
  long long media_start_ms;
  long long last_refresh_retry_ms;
  long long last_pli_ms;
...
static struct ard_hp_udp_media_state g_udp_media = {
    .video_sock = -1,
    .audio_sock = -1,
};

static char g_media_dump_dir[PATH_MAX];
static int g_media_dump_enabled = 0;
static unsigned long g_media_dump_packet_index = 0;
static unsigned long g_media_dump_au_index = 0;
static unsigned long g_media_dump_decrypted_all_index = 0;
static char *g_last_media_options_hex = NULL;

static void ard_hp_set_last_media_options_hex(const char *hex) {
  free(g_last_media_options_hex);
  g_last_media_options_hex = NULL;
  if (!hex || !*hex) return;
  g_last_media_options_hex = strdup(hex);
}

static void ard_hp_media_dump_init(void) {
  const char *env = getenv("VNC_ARD_HP_DUMP_DIR");
  struct stat st;
  if (g_media_dump_dir[0] != '\0') return;
  if (!env || !*env) return;
  if (snprintf(g_media_dump_dir, sizeof(g_media_dump_dir), "%s", env) <= 0) return;
  if (stat(g_media_dump_dir, &st) != 0) {
    if (mkdir(g_media_dump_dir, 0755) != 0) {
      rfbClientErr("ard-hp: failed to create dump dir '%s' (%d: %s)\n",
                   g_media_dump_dir, errno, strerror(errno));
      g_media_dump_dir[0] = '\0';
      return;
    }
  } else if (!S_ISDIR(st.st_mode)) {
    rfbClientErr("ard-hp: dump path is not a directory: %s\n", g_media_dump_dir);
    g_media_dump_dir[0] = '\0';
    return;
  }
  g_media_dump_enabled = 1;
  rfbClientLog("ard-hp: media dump enabled dir=%s\n", g_media_dump_dir);
}

static void ard_hp_media_dump_write(const char *kind, unsigned long index, const uint8_t *buf, size_t len) {
  char path[PATH_MAX];
  FILE *f;
  if (!g_media_dump_enabled || !kind || !buf || len == 0) return;
  if (snprintf(path, sizeof(path), "%s/%s_%08lu.bin", g_media_dump_dir, kind, index) <= 0) return;
  f = fopen(path, "wb");
  if (!f) return;
...
#if defined(ARDHPDEBUG_HAS_FFMPEG)
#define ARD_HP_SRTP_CTX_MAX 16u
#define ARD_HP_RTP_SSRC_TRACK_MAX 16u
#define ARD_HP_ADAPTIVE_LANE_DECODER_MAX 8u
#define ARD_HP_ADAPTIVE_FU_CTX_MAX 8u
#define ARD_HP_ADAPTIVE_FU_PENDING_MAX 64u
enum ard_hp_video_decoder_backend {
  ARD_HP_DECODER_BACKEND_NONE = 0,
  ARD_HP_DECODER_BACKEND_FFMPEG = 1,
  ARD_HP_DECODER_BACKEND_VIDEOTOOLBOX = 2,
};

struct ard_hp_rtp_ssrc_track {
  uint32_t ssrc;
...
struct ard_hp_adaptive_fu_ctx {
  uint32_t ssrc;
  int active;
  uint16_t next_seq;
  uint8_t hevc_hdr_off;
  uint8_t hevc_donl_skip;
  struct {
    uint16_t seq;
    uint8_t *data;
    size_t len;
    int end;
    int used;
  } pending[ARD_HP_ADAPTIVE_FU_PENDING_MAX];
  uint8_t *buf;
  size_t len;
  size_t cap;
};

struct ard_hp_video_decoder_state {
  int backend;
  AVCodecContext *codec_ctx;
  ...
  size_t rtp_au_cap;
  uint32_t rtp_au_ts;
  uint32_t rtp_au_ssrc;
  int rtp_au_active;
  int rtp_au_codec;
  uint16_t rtp_fu_next_seq;
  uint32_t rtp_fu_ssrc;
  int rtp_fu_active;
  int rtp_fu_codec;
  uint8_t rtp_fu_hevc_hdr_off;
  uint8_t rtp_fu_hevc_donl_skip;
  unsigned long rtp_fu_completed;
  unsigned long rtp_fu_dropped;
  unsigned long rtp_au_completed;
  unsigned long rtp_au_dropped;
  rfbARDHPSRTPContext *srtp_ctx[ARD_HP_SRTP_CTX_MAX];
  rfbARDHPSRTPContext *srtp_tx_ctx[ARD_HP_SRTP_CTX_MAX];
  unsigned long srtp_ctx_count;
  unsigned long srtp_tx_ctx_count;
  int srtp_init_attempted;
  int srtp_contexts_logged;
  unsigned long srtp_unprotect_ok;
  unsigned long srtp_unprotect_fail;
  unsigned long srtcp_protect_ok;
  unsigned long srtcp_protect_fail;
  unsigned long srtp_last_rx_ctx_idx;
  int srtp_last_rx_ctx_valid;
  unsigned long srtcp_preferred_tx_ctx_idx;
  int srtcp_preferred_tx_ctx_valid;
  unsigned long rtp_probe_logged;
  uint32_t rtp_selected_ssrc;
  int rtp_selected_ssrc_valid;
  unsigned long rtp_selected_packets;
  unsigned long rtp_dropped_other_ssrc;
  unsigned long rtp_selected_frames_at_lock;
  struct ard_hp_rtp_ssrc_track rtp_ssrc_tracks[ARD_HP_RTP_SSRC_TRACK_MAX];
  unsigned long rtp_ssrc_track_count;
  unsigned long rtp_ssrc_overflow_packets;
  uint32_t adaptive_lane_ssrc[ARD_HP_RTP_SSRC_TRACK_MAX];
  unsigned long adaptive_lane_count;
  int adaptive_multistream_enabled;
  struct SwsContext *strip_sws;
  int strip_sws_src_w;
  int strip_sws_src_h;
  uint8_t *strip_bgra_buf;
  size_t strip_bgra_cap;
  uint32_t strip_last_ssrc;
  unsigned long strip_frames_composited;
  struct ard_hp_lane_decoder lane_decoder[ARD_HP_ADAPTIVE_LANE_DECODER_MAX];
...
static struct ard_hp_video_decoder_state g_video = {0};
#endif
...
static int ard_hp_send_adaptive_media_options_after_stage1(rfbClient *client);
...
#endif
static void ard_hp_media_udp_shutdown(void);
static int ard_hp_media_udp_maybe_start(rfbClient *client);
static int ard_hp_media_udp_pump(rfbClient *client);
static int ard_hp_decode_video_media_packet(rfbClient *client, uint32_t encoding,
                                           const uint8_t *payload, size_t total_len);
static void ard_hp_shutdown_video_decoder(void);
#if defined(ARDHPDEBUG_HAS_FFMPEG)
static void ard_hp_rtp_fu_reset_state(int dropped);
static void ard_hp_rtp_au_reset_state(int dropped);
static int ard_hp_push_decoded_frame(rfbClient *client, AVFrame *frame,
                                     uint32_t rtp_ssrc, int adaptive_strip_mode);
#endif
...
static int ard_hp_strict_stage2_mode(void) {
  return env_flag_enabled("VNC_ARD_HP_STRICT_STAGE2");
}

static uint16_t env_u16_or_default(const char *name, uint16_t default_value) {
  const char *s;
  char *end = NULL;
  long parsed;

  s = ard_hp_getenv_compat(name);
  if (!s || !*s) return default_value;
  errno = 0;
  parsed = strtol(s, &end, 10);
  if (errno != 0 || !end || *end != '\0' || parsed < 0 || parsed > 65535) {
    rfbClientErr("ard-hp: ignoring invalid %s='%s' (expected uint16)\n", name, s);
    return default_value;
  }
  return (uint16_t)parsed;
}

static int env_u32_optional(const char *name, uint32_t *out_value) {
  const char *s;
  char *end = NULL;
  unsigned long parsed;
  if (out_value) *out_value = 0;
  s = ard_hp_getenv_compat(name);
  if (!s || !*s || !out_value) return 0;
  errno = 0;
  parsed = strtoul(s, &end, 0);
  if (errno != 0 || !end || *end != '\0' || parsed > 0xffffffffUL) {
    rfbClientErr("ard-hp: ignoring invalid %s='%s' (expected uint32, decimal or 0x...)\n", name, s);
    return 0;
  }
...
static int ard_hp_send_keyboard_source_share(rfbClient *client, const char *source_id, int shared) {
  static const uint8_t kDisableMsg[8] = {0x1a, 0x00, 0x00, 0x00, 0x04, 0x00, 0x01, 0x00};
  uint8_t *msg = NULL;
  size_t source_len = 0;
  size_t msg_len = 0;
  int ok = 0;
  ...
  if (!client) return 0;
  if (!shared || !source_id || !*source_id) {
    rfbClientLog("ard-hp: sending KeyboardSourceShare disable (8 bytes)\n");
    return WriteToRFBServer(client, (const char *)kDisableMsg, sizeof(kDisableMsg));
  }

  source_len = strlen(source_id);
  if (source_len > 0xfff0) source_len = 0xfff0;
  msg_len = source_len + 8u;
  msg = (uint8_t *)calloc(1, msg_len);
  if (!msg) return 0;

  msg[0] = 0x1a;
  msg[2] = (uint8_t)(((source_len + 4u) >> 8) & 0xff);
  msg[3] = (uint8_t)((source_len + 4u) & 0xff);
  msg[4] = 0x00;
  msg[5] = 0x01;
  msg[6] = (uint8_t)((source_len >> 8) & 0xff);
  msg[7] = (uint8_t)(source_len & 0xff);
  memcpy(msg + 8, source_id, source_len);

  rfbClientLog("ard-hp: sending KeyboardSourceShare enable source='%s' len=%lu\n",
               source_id, (unsigned long)source_len);
  ok = WriteToRFBServer(client, (const char *)msg, (unsigned int)msg_len);
  free(msg);
  return ok;
}
...
static int ard_hp_probe_h264_annexb(const uint8_t *buf, size_t len,
                                    unsigned *out_start_codes,
                                    unsigned *out_sps,
                                    unsigned *out_pps,
                                    unsigned *out_idr) {
  size_t i;
  unsigned start_codes = 0;
  unsigned sps = 0;
  unsigned pps = 0;
  unsigned idr = 0;
  ...
  for (i = 0; i + 4 < len; ++i) {
    ...
    if (buf[i] == 0x00 && buf[i + 1] == 0x00 && buf[i + 2] == 0x01) {
      nal_off = i + 3;
    } else if (i + 5 < len && buf[i] == 0x00 && buf[i + 1] == 0x00 &&
    ...
    nal = buf[nal_off];
    nal_type = (uint8_t)(nal & 0x1f);
    if (nal_type == 0 || nal_type > 23) continue;
    start_codes++;
    if (nal_type == 7) sps++;
    if (nal_type == 8) pps++;
    if (nal_type == 5) idr++;
  ...
  if (out_start_codes) *out_start_codes = start_codes;
  if (out_sps) *out_sps = sps;
  if (out_pps) *out_pps = pps;
  if (out_idr) *out_idr = idr;
  if (start_codes == 0) return 0;
  if ((sps > 0 && pps > 0) || idr > 0) return 1;
  return start_codes >= 3;
}

static int ard_hp_probe_h264_avcc(const uint8_t *buf, size_t len,
                                  unsigned *out_nals,
                                  unsigned *out_sps,
                                  unsigned *out_pps,
                                  unsigned *out_idr) {
  ...
  while (off + 4 <= len) {
    uint32_t nal_len = read_be_u32(buf + off);
    uint8_t nal_type;

    off += 4;
    if (nal_len == 0 || off + nal_len > len) break;
    nal_type = (uint8_t)(buf[off] & 0x1f);
...
static void ard_hp_log_media_codec_probe(uint32_t encoding, const uint8_t *payload, size_t total_len) {
  const uint8_t *body = payload;
  size_t body_len = total_len;
  unsigned annexb_start_codes = 0;
  unsigned annexb_sps = 0;
  unsigned annexb_pps = 0;
  unsigned annexb_idr = 0;
  unsigned avcc_nals = 0;
  unsigned avcc_sps = 0;
  unsigned avcc_pps = 0;
  unsigned avcc_idr = 0;
  int annexb_hit;
  ...
  if (!payload || total_len == 0) return;
  if (total_len >= 2) {
    uint16_t declared = read_be_u16(payload);
    if ((size_t)declared + 2 <= total_len) {
      body = payload + 2;
      body_len = declared;
    }
  }

  annexb_hit = ard_hp_probe_h264_annexb(body, body_len,
                                        &annexb_start_codes,
                                        &annexb_sps,
                                        &annexb_pps,
                                        &annexb_idr);
  avcc_hit = ard_hp_probe_h264_avcc(body, body_len,
                                    &avcc_nals,
                                    &avcc_sps,
                                    &avcc_pps,
                                    &avcc_idr);
  likely_h264 = annexb_hit || avcc_hit;
  if (annexb_hit) fmt = "annexb";
  else if (avcc_hit) fmt = "avcc";

  rfbClientLog(
      "ard-hp: codec-probe encoding=0x%08x likely_h264=%s format=%s body_len=%lu annexb(start=%u sps=%u pps=%u idr=%u) avcc(nals=%u sps=%u pps=%u idr=%u)\n",
      (unsigned)encoding,
      likely_h264 ? "yes" : "no",
      fmt,
...
#if defined(ARDHPDEBUG_HAS_FFMPEG)
static void ard_hp_log_av_error(const char *prefix, int errnum) {
  char errbuf[128];
  errbuf[0] = '\0';
  av_strerror(errnum, errbuf, sizeof(errbuf));
  rfbClientErr("ard-hp: %s: %s (%d)\n", prefix, errbuf, errnum);
}

enum ard_hp_video_codec {
  ARD_HP_VIDEO_CODEC_UNKNOWN = 0,
  ARD_HP_VIDEO_CODEC_HEVC = 1,
  ARD_HP_VIDEO_CODEC_H264 = 2,
};

static const char *ard_hp_video_codec_name(enum ard_hp_video_codec codec) {
  switch (codec) {
    case ARD_HP_VIDEO_CODEC_HEVC: return "hevc";
    case ARD_HP_VIDEO_CODEC_H264: return "h264";
    default: return "unknown";
  }
}

static enum AVCodecID ard_hp_video_codec_to_ffmpeg_id(enum ard_hp_video_codec codec) {
  switch (codec) {
    case ARD_HP_VIDEO_CODEC_HEVC: return AV_CODEC_ID_HEVC;
    case ARD_HP_VIDEO_CODEC_H264: return AV_CODEC_ID_H264;
    default: return AV_CODEC_ID_NONE;
  }
}

static int ard_hp_is_start_code3(const uint8_t *p, size_t remain) {
  return remain >= 3u && p[0] == 0x00 && p[1] == 0x00 && p[2] == 0x01;
}

static int ard_hp_is_start_code4(const uint8_t *p, size_t remain) {
  return remain >= 4u && p[0] == 0x00 && p[1] == 0x00 && p[2] == 0x00 && p[3] == 0x01;
}

static int ard_hp_h264_find_nal(const uint8_t *annexb, size_t len, size_t *cursor,
                                const uint8_t **out_nal, size_t *out_nal_len) {
  size_t i;
  size_t start = 0;
  size_t nal_start;
  size_t nal_end;
  if (!annexb || !cursor || !out_nal || !out_nal_len) return 0;
  i = *cursor;
  while (i + 3u < len) {
    if (ard_hp_is_start_code4(annexb + i, len - i)) {
      start = i;
      i += 4u;
      break;
    }
    if (ard_hp_is_start_code3(annexb + i, len - i)) {
      start = i;
      i += 3u;
      break;
    }
  ...
  nal_end = len;
  while (i + 3u < len) {
    if (ard_hp_is_start_code4(annexb + i, len - i) || ard_hp_is_start_code3(annexb + i, len - i)) {
      nal_end = i;
      break;
    }
    i++;
  }
  while (nal_end > nal_start && annexb[nal_end - 1u] == 0x00) nal_end--;
  if (nal_end <= nal_start) {
    *cursor = (start + 1u < len) ? (start + 1u) : len;
    return 0;
  }
  *out_nal = annexb + nal_start;
  *out_nal_len = nal_end - nal_start;
  *cursor = nal_end;
  return 1;
}

#if defined(__APPLE__) && defined(ARDHPDEBUG_HAS_VIDEOTOOLBOX)
static int ard_hp_should_use_videotoolbox(enum ard_hp_video_codec codec) {
  const char *env;
  if (codec != ARD_HP_VIDEO_CODEC_H264) return 0;
  env = ard_hp_getenv_compat("VNC_ARD_HP_VIDEO_DECODER");
  if (!env || !*env || strcmp(env, "auto") == 0) return 1;
  if (strcmp(env, "videotoolbox") == 0 || strcmp(env, "vt") == 0) return 1;
  return 0;
}

static void ard_hp_vt_release_decoder(void) {
  if (g_video.vt_session) {
    VTDecompressionSessionInvalidate(g_video.vt_session);
    CFRelease(g_video.vt_session);
    g_video.vt_session = NULL;
  }
  if (g_video.vt_format_desc) {
    CFRelease(g_video.vt_format_desc);
    g_video.vt_format_desc = NULL;
  }
  free(g_video.vt_sps);
  g_video.vt_sps = NULL;
  g_video.vt_sps_len = 0;
  free(g_video.vt_pps);
  g_video.vt_pps = NULL;
  g_video.vt_pps_len = 0;
}

static int ard_hp_vt_update_parameter_set(uint8_t **dst, size_t *dst_len,
                                          const uint8_t *src, size_t src_len) {
  uint8_t *buf;
  if (!dst || !dst_len || !src || src_len == 0) return 0;
  if (*dst && *dst_len == src_len && memcmp(*dst, src, src_len) == 0) return 0;
...
struct ard_hp_vt_decode_ctx {
  rfbClient *client;
  int decoded_now;
};

static int ard_hp_push_vt_image(rfbClient *client, CVImageBufferRef image) {
  ...
  size_t row_copy;
  size_t y;
  if (!client || !pb) return 0;
  if (CVPixelBufferGetPixelFormatType(pb) != kCVPixelFormatType_32BGRA) return 0;
  src_w = CVPixelBufferGetWidth(pb);
  src_h = CVPixelBufferGetHeight(pb);
  if (src_w == 0 || src_h == 0) return 0;
  if (!ard_hp_resize_framebuffer_if_needed(client, (uint16_t)src_w, (uint16_t)src_h)) return 0;
  if (CVPixelBufferLockBaseAddress(pb, kCVPixelBufferLock_ReadOnly) != kCVReturnSuccess) return 0;
  base = (uint8_t *)CVPixelBufferGetBaseAddress(pb);
  src_stride = CVPixelBufferGetBytesPerRow(pb);
  dst_stride = (size_t)client->width * 4u;
  row_copy = dst_stride < src_stride ? dst_stride : src_stride;
  if (!base || !client->frameBuffer) {
    CVPixelBufferUnlockBaseAddress(pb, kCVPixelBufferLock_ReadOnly);
    return 0;
  }
  for (y = 0; y < src_h; ++y) {
    memcpy(client->frameBuffer + y * dst_stride, base + y * src_stride, row_copy);
  }
  CVPixelBufferUnlockBaseAddress(pb, kCVPixelBufferLock_ReadOnly);
  on_fb_update(client, 0, 0, (int)src_w, (int)src_h);
  on_fb_update_done(client);
  return 1;
}

static void ard_hp_vt_output_cb(void *decompressionOutputRefCon,
  ...
  struct ard_hp_vt_decode_ctx *ctx = (struct ard_hp_vt_decode_ctx *)sourceFrameRefCon;
  ...
  if (ard_hp_push_vt_image(ctx->client, imageBuffer)) {
    g_video.frames_decoded++;
    ctx->decoded_now++;
  }
}

static int ard_hp_vt_rebuild_session(void) {
  const uint8_t *param_sets[2];
  size_t param_sizes[2];
  CFMutableDictionaryRef attrs = NULL;
  CFNumberRef pixfmt = NULL;
  OSType fmt = kCVPixelFormatType_32BGRA;
  VTDecompressionOutputCallbackRecord cb = {0};
  OSStatus st;
  if (!g_video.vt_sps || !g_video.vt_pps || g_video.vt_sps_len == 0 || g_video.vt_pps_len == 0) return 0;
  if (g_video.vt_session) {
    VTDecompressionSessionInvalidate(g_video.vt_session);
    CFRelease(g_video.vt_session);
    g_video.vt_session = NULL;
  ...
  param_sets[0] = g_video.vt_sps;
  param_sets[1] = g_video.vt_pps;
  param_sizes[0] = g_video.vt_sps_len;
  param_sizes[1] = g_video.vt_pps_len;
  st = CMVideoFormatDescriptionCreateFromH264ParameterSets(
      kCFAllocatorDefault, 2, param_sets, param_sizes, 4, &g_video.vt_format_desc);
  if (st != noErr || !g_video.vt_format_desc) return 0;

  attrs = CFDictionaryCreateMutable(kCFAllocatorDefault, 1, &kCFTypeDictionaryKeyCallBacks,
                                    &kCFTypeDictionaryValueCallBacks);
  if (!attrs) return 0;
  pixfmt = CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &fmt);
  if (pixfmt) {
    CFDictionarySetValue(attrs, kCVPixelBufferPixelFormatTypeKey, pixfmt);
    CFRelease(pixfmt);
  }
  cb.decompressionOutputCallback = ard_hp_vt_output_cb;
  cb.decompressionOutputRefCon = NULL;
  st = VTDecompressionSessionCreate(kCFAllocatorDefault, g_video.vt_format_desc,
                                    NULL, attrs, &cb, &g_video.vt_session);
  CFRelease(attrs);
  if (st != noErr || !g_video.vt_session) return 0;
  return 1;
}

static int ard_hp_vt_decode_h264_annexb(rfbClient *client, const uint8_t *annexb, size_t annexb_len,
                                        int *decoded_now) {
  size_t cursor = 0;
  size_t avcc_cap = 0;
  size_t avcc_len = 0;
  ...
  int have_slice = 0;
  int fmt_changed = 0;
  struct ard_hp_vt_decode_ctx ctx;
  CMBlockBufferRef block = NULL;
  CMSampleBufferRef sample = NULL;
  OSStatus st;

  if (decoded_now) *decoded_now = 0;
  if (!client || !annexb || annexb_len == 0) return 0;

  while (cursor < annexb_len) {
    const uint8_t *nal = NULL;
    size_t nal_len = 0;
    uint8_t nal_type;
    size_t need;
    if (!ard_hp_h264_find_nal(annexb, annexb_len, &cursor, &nal, &nal_len)) continue;
    if (!nal || nal_len == 0) continue;
    nal_type = (uint8_t)(nal[0] & 0x1fu);
    if (nal_type == 7u) {
      if (ard_hp_vt_update_parameter_set(&g_video.vt_sps, &g_video.vt_sps_len, nal, nal_len)) fmt_changed = 1;
      continue;
    }
    if (nal_type == 8u) {
      if (ard_hp_vt_update_parameter_set(&g_video.vt_pps, &g_video.vt_pps_len, nal, nal_len)) fmt_changed = 1;
      continue;
    }
    need = avcc_len + 4u + nal_len;
    if (need > avcc_cap) {
      size_t next = avcc_cap == 0 ? 4096u : avcc_cap;
      uint8_t *grown;
      while (next < need) next *= 2u;
      grown = (uint8_t *)realloc(avcc, next);
      if (!grown) goto done;
      avcc = grown;
      avcc_cap = next;
    }
    avcc[avcc_len + 0] = (uint8_t)((nal_len >> 24) & 0xffu);
    avcc[avcc_len + 1] = (uint8_t)((nal_len >> 16) & 0xffu);
    avcc[avcc_len + 2] = (uint8_t)((nal_len >> 8) & 0xffu);
    avcc[avcc_len + 3] = (uint8_t)(nal_len & 0xffu);
    memcpy(avcc + avcc_len + 4u, nal, nal_len);
    avcc_len += 4u + nal_len;
    if (nal_type == 1u || nal_type == 2u || nal_type == 3u || nal_type == 4u || nal_type == 5u) {
      have_slice = 1;
    }
  }

  if (fmt_changed || !g_video.vt_session) {
    if (!ard_hp_vt_rebuild_session()) goto done;
  }
  if (!g_video.vt_session || !g_video.vt_format_desc || !have_slice || avcc_len == 0) goto done;

  st = CMBlockBufferCreateWithMemoryBlock(kCFAllocatorDefault, NULL, avcc_len, kCFAllocatorDefault,
                                          NULL, 0, avcc_len, 0, &block);
  if (st != kCMBlockBufferNoErr || !block) goto done;
  st = CMBlockBufferReplaceDataBytes(avcc, block, 0, avcc_len);
  if (st != kCMBlockBufferNoErr) goto done;
  {
    const size_t sample_size = avcc_len;
    st = CMSampleBufferCreateReady(kCFAllocatorDefault, block, g_video.vt_format_desc, 1, 0,
                                   NULL, 1, &sample_size, &sample);
  }
  if (st != noErr || !sample) goto done;

  ctx.client = client;
  ctx.decoded_now = 0;
  st = VTDecompressionSessionDecodeFrame(g_video.vt_session, sample, 0, &ctx, NULL);
  if (st == noErr) {
    (void)VTDecompressionSessionWaitForAsynchronousFrames(g_video.vt_session);
    if (decoded_now) *decoded_now = ctx.decoded_now;
    if (ctx.decoded_now > 0) {
...
static int ard_hp_append_annexb_nal(uint8_t **buf, size_t *len, size_t *cap,
                                    const uint8_t *nal, size_t nal_len) {
  size_t need;
  uint8_t *grown;
  if (!buf || !len || !cap || !nal || nal_len == 0) return 0;
  need = *len + 4u + nal_len;
  if (need > *cap) {
    size_t next_cap = (*cap == 0) ? 4096u : *cap;
    while (next_cap < need) {
      if (next_cap > (SIZE_MAX / 2u)) return 0;
      next_cap *= 2u;
    }
    grown = (uint8_t *)realloc(*buf, next_cap);
    if (!grown) return 0;
    *buf = grown;
    *cap = next_cap;
  }
  ...
  (*buf)[*len + 2] = 0x00;
  (*buf)[*len + 3] = 0x01;
  memcpy(*buf + *len + 4u, nal, nal_len);
  *len += 4u + nal_len;
  return 1;
}

static void ard_hp_store_ps(uint8_t **dst, size_t *dst_len, const uint8_t *src, size_t src_len) {
  uint8_t *copy;
  if (!dst || !dst_len || !src || src_len == 0) return;
...
static int ard_hp_next_annexb_nal(const uint8_t *buf, size_t len, size_t *io_off,
                                  const uint8_t **out_nal, size_t *out_nal_len) {
  size_t i;
  size_t start = (size_t)-1;
  size_t sc_len = 0;
  size_t end;
  if (!buf || !io_off || !out_nal || !out_nal_len) return 0;
  *out_nal = NULL;
  *out_nal_len = 0;
  i = *io_off;
  while (i + 3u < len) {
    ...
    if (i + 4u < len &&
        buf[i] == 0x00 && buf[i + 1u] == 0x00 &&
        buf[i + 2u] == 0x00 && buf[i + 3u] == 0x01) {
      start = i + 4u;
      sc_len = 4u;
      break;
    }
  ...
  if (start == (size_t)-1 || start >= len) {
    *io_off = len;
    return 0;
  }
  end = start;
  while (end + 3u < len) {
    if (buf[end] == 0x00 && buf[end + 1u] == 0x00 &&
        ((buf[end + 2u] == 0x01) ||
         (end + 4u < len && buf[end + 2u] == 0x00 && buf[end + 3u] == 0x01))) {
      break;
    }
    end++;
  }
  ...
  *out_nal = buf + start;
  *out_nal_len = end - start;
  *io_off = end;
  return 1;
}

static void ard_hp_update_param_sets_from_annexb(enum ard_hp_video_codec codec,
                                                 const uint8_t *buf, size_t len) {
  size_t off = 0;
  const uint8_t *nal = NULL;
  size_t nal_len = 0;
  if (!buf || len == 0) return;
  while (ard_hp_next_annexb_nal(buf, len, &off, &nal, &nal_len)) {
    if (!nal || nal_len == 0) continue;
    if (codec == ARD_HP_VIDEO_CODEC_HEVC && nal_len >= 2u) {
      uint8_t nalu_type = (uint8_t)((nal[0] >> 1) & 0x3f);
      if (nalu_type == 32u) ard_hp_store_ps(&g_video.hevc_vps, &g_video.hevc_vps_len, nal, nal_len);
      else if (nalu_type == 33u) ard_hp_store_ps(&g_video.hevc_sps, &g_video.hevc_sps_len, nal, nal_len);
      else if (nalu_type == 34u) ard_hp_store_ps(&g_video.hevc_pps, &g_video.hevc_pps_len, nal, nal_len);
    }
  }
}

static int ard_hp_prepend_param_sets_if_needed(enum ard_hp_video_codec codec,
                                               uint8_t **io_buf, size_t *io_len) {
  size_t off = 0;
  const uint8_t *nal = NULL;
  size_t nal_len = 0;
  int has_vcl = 0;
  ...
  size_t merged_cap = 0;
  uint8_t *old;
  if (!io_buf || !*io_buf || !io_len || *io_len == 0) return 1;
  if (codec != ARD_HP_VIDEO_CODEC_HEVC) return 1;
  while (ard_hp_next_annexb_nal(*io_buf, *io_len, &off, &nal, &nal_len)) {
    uint8_t t;
    if (!nal || nal_len < 2u) continue;
    t = (uint8_t)((nal[0] >> 1) & 0x3f);
    if (t <= 31u) has_vcl = 1;
    else if (t == 32u) has_vps = 1;
    else if (t == 33u) has_sps = 1;
    else if (t == 34u) has_pps = 1;
  }
  if (!has_vcl) return 1;
  if (has_vps && has_sps && has_pps) return 1;
  if (!g_video.hevc_vps || !g_video.hevc_sps || !g_video.hevc_pps) return 1;
  if (!ard_hp_append_annexb_nal(&merged, &merged_len, &merged_cap, g_video.hevc_vps, g_video.hevc_vps_len) ||
      !ard_hp_append_annexb_nal(&merged, &merged_len, &merged_cap, g_video.hevc_sps, g_video.hevc_sps_len) ||
      !ard_hp_append_annexb_nal(&merged, &merged_len, &merged_cap, g_video.hevc_pps, g_video.hevc_pps_len)) {
    free(merged);
    return 0;
  }
  if (merged_len + *io_len > merged_cap) {
    uint8_t *grown = (uint8_t *)realloc(merged, merged_len + *io_len);
    if (!grown) {
      free(merged);
      return 0;
    }
  ...
  memcpy(merged + merged_len, *io_buf, *io_len);
  merged_len += *io_len;
  old = *io_buf;
  *io_buf = merged;
  *io_len = merged_len;
...
static int ard_hp_parse_rtp_payload(const uint8_t *packet, size_t packet_len,
                                    const uint8_t **out_payload, size_t *out_payload_len,
                                    uint16_t *out_seq, uint32_t *out_ssrc,
                                    uint32_t *out_timestamp, uint8_t *out_marker,
                                    uint8_t *out_pt) {
  uint8_t b0;
  uint8_t b1;
  size_t off;
  ...
  b1 = packet[1];
  if ((b0 >> 6) != 2) return 0;
  /* Drop RTCP packets (e.g. SR=200/RR=201) that share the same UDP flow. */
  if (b1 >= 200u && b1 <= 223u && packet_len >= 4u) {
    size_t rtcp_len = ((size_t)read_be_u16(packet + 2u) + 1u) * 4u;
    if (rtcp_len <= packet_len) return 0;
  }
  cc = (uint8_t)(b0 & 0x0f);
  off = 12u + ((size_t)cc * 4u);
  ...
  if (b0 & 0x20u) {
    uint8_t pad = packet[packet_len - 1u];
    if (pad == 0 || (size_t)pad > (packet_len - off)) return 0;
    end -= (size_t)pad;
  }
  if (end <= off) return 0;
  if (out_seq) *out_seq = read_be_u16(packet + 2u);
  if (out_ssrc) *out_ssrc = read_be_u32(packet + 8u);
  if (out_timestamp) *out_timestamp = read_be_u32(packet + 4u);
  if (out_marker) *out_marker = (uint8_t)((b1 & 0x80u) ? 1u : 0u);
  if (out_pt) *out_pt = (uint8_t)(b1 & 0x7fu);
  *out_payload = packet + off;
  *out_payload_len = end - off;
  return 1;
}

static struct ard_hp_rtp_ssrc_track *ard_hp_find_rtp_ssrc_track(uint32_t ssrc, int create) {
  unsigned long i;
  for (i = 0; i < g_video.rtp_ssrc_track_count; ++i) {
    if (g_video.rtp_ssrc_tracks[i].ssrc == ssrc) return &g_video.rtp_ssrc_tracks[i];
  }
  if (!create) return NULL;
  if (g_video.rtp_ssrc_track_count >= ARD_HP_RTP_SSRC_TRACK_MAX) {
    g_video.rtp_ssrc_overflow_packets++;
    return NULL;
  }
  i = g_video.rtp_ssrc_track_count++;
  memset(&g_video.rtp_ssrc_tracks[i], 0, sizeof(g_video.rtp_ssrc_tracks[i]));
  g_video.rtp_ssrc_tracks[i].ssrc = ssrc;
  return &g_video.rtp_ssrc_tracks[i];
}

static void ard_hp_probe_annexb_key_units(enum ard_hp_video_codec codec,
                                          const uint8_t *buf, size_t len,
                                          unsigned long *out_ps_units,
                                          unsigned long *out_idr_units) {
  size_t off = 0;
  const uint8_t *nal = NULL;
  size_t nal_len = 0;
  unsigned long ps = 0;
  unsigned long idr = 0;
  if (out_ps_units) *out_ps_units = 0;
  if (out_idr_units) *out_idr_units = 0;
  if (!buf || len == 0) return;

  while (ard_hp_next_annexb_nal(buf, len, &off, &nal, &nal_len)) {
    if (!nal || nal_len == 0) continue;
    if ((codec == ARD_HP_VIDEO_CODEC_HEVC || codec == ARD_HP_VIDEO_CODEC_UNKNOWN) && nal_len >= 2u) {
      uint8_t hevc_type = (uint8_t)((nal[0] >> 1) & 0x3f);
      if (hevc_type == 32u || hevc_type == 33u || hevc_type == 34u) ps++;
      if (hevc_type == 19u || hevc_type == 20u) idr++;
    }
    if (codec == ARD_HP_VIDEO_CODEC_H264 || codec == ARD_HP_VIDEO_CODEC_UNKNOWN) {
      uint8_t h264_type = (uint8_t)(nal[0] & 0x1fu);
      if (h264_type == 7u || h264_type == 8u) ps++;
      if (h264_type == 5u) idr++;
    }
  }
  if (out_ps_units) *out_ps_units = ps;
  if (out_idr_units) *out_idr_units = idr;
}

static int ard_hp_get_forced_video_ssrc(uint32_t *out_ssrc) {
  static int parsed = 0;
  static int has_value = 0;
  static uint32_t forced = 0;
  if (!parsed) {
    parsed = 1;
    has_value = env_u32_optional("VNC_ARD_HP_VIDEO_SSRC", &forced);
    if (has_value) {
      rfbClientLog("ard-hp: forcing RTP video SSRC=0x%08x via VNC_ARD_HP_VIDEO_SSRC\n",
                   (unsigned)forced);
    }
  }
  if (out_ssrc) *out_ssrc = forced;
  return has_value;
}

static void ard_hp_note_rtp_ssrc_packet(uint32_t ssrc, enum ard_hp_video_codec codec,
                                        const uint8_t *annexb, size_t annexb_len) {
  struct ard_hp_rtp_ssrc_track *track = ard_hp_find_rtp_ssrc_track(ssrc, 1);
  unsigned long ps_units = 0;
  unsigned long idr_units = 0;
  if (!track) return;
  track->packets++;
  ard_hp_probe_annexb_key_units(codec, annexb, annexb_len, &ps_units, &idr_units);
  track->ps_units += ps_units;
  track->idr_units += idr_units;
}

static uint32_t ard_hp_choose_rtp_ssrc_candidate(int require_idr) {
  unsigned long i;
  uint32_t best_ssrc = 0;
  unsigned long long best_score = 0;
  for (i = 0; i < g_video.rtp_ssrc_track_count; ++i) {
    const struct ard_hp_rtp_ssrc_track *t = &g_video.rtp_ssrc_tracks[i];
    unsigned long long score;
    if (t->ssrc == 0 || t->packets == 0) continue;
    if (require_idr && t->idr_units == 0) continue;
    score = (unsigned long long)t->packets * 1000ULL +
            (unsigned long long)t->au_emitted * 100ULL +
            (unsigned long long)t->ps_units * 20ULL +
            (unsigned long long)t->idr_units * 5ULL;
    if (best_ssrc == 0 || score > best_score) {
      best_ssrc = t->ssrc;
      best_score = score;
    }
  }
...
static void ard_hp_lock_rtp_video_ssrc(uint32_t ssrc, const char *reason) {
  if (ssrc == 0) return;
  if (g_video.rtp_selected_ssrc_valid && g_video.rtp_selected_ssrc == ssrc) return;
  g_video.rtp_selected_ssrc = ssrc;
  g_video.rtp_selected_ssrc_valid = 1;
  g_video.rtp_selected_packets = 0;
  g_video.rtp_selected_frames_at_lock = g_video.frames_decoded;
  ard_hp_rtp_fu_reset_state(0);
  ard_hp_rtp_au_reset_state(0);
  rfbClientLog("ard-hp: locked RTP video SSRC=0x%08x (%s)\n",
               (unsigned)ssrc, reason ? reason : "auto");
}

static int ard_hp_should_accept_rtp_ssrc(uint32_t ssrc) {
  unsigned long i;
  unsigned long total_packets = 0;
  uint32_t forced_ssrc = 0;

  if (ssrc == 0) return 1;
  if (ard_hp_get_forced_video_ssrc(&forced_ssrc)) {
    if (!g_video.rtp_selected_ssrc_valid && ssrc == forced_ssrc) {
      ard_hp_lock_rtp_video_ssrc(ssrc, "forced");
    }
    if (ssrc != forced_ssrc) {
      g_video.rtp_dropped_other_ssrc++;
      return 0;
    }
  }

  if (g_video.rtp_selected_ssrc_valid) {
    if (ssrc != g_video.rtp_selected_ssrc) {
      g_video.rtp_dropped_other_ssrc++;
      return 0;
    }
    g_video.rtp_selected_packets++;
    return 1;
  }

  for (i = 0; i < g_video.rtp_ssrc_track_count; ++i) total_packets += g_video.rtp_ssrc_tracks[i].packets;
  if (total_packets >= 256u) {
    uint32_t candidate = ard_hp_choose_rtp_ssrc_candidate(0);
    if (candidate != 0) {
      ard_hp_lock_rtp_video_ssrc(candidate, "scored-candidate");
      if (ssrc == candidate) {
        g_video.rtp_selected_packets++;
        return 1;
      }
...
static void ard_hp_note_rtp_ssrc_au(uint32_t ssrc) {
  struct ard_hp_rtp_ssrc_track *track;
  if (ssrc == 0) return;
  track = ard_hp_find_rtp_ssrc_track(ssrc, 0);
  if (!track) return;
  track->au_emitted++;
}

static void ard_hp_rtp_fu_reset_state(int dropped) {
  if (dropped) g_video.rtp_fu_dropped++;
  g_video.rtp_fu_active = 0;
  g_video.rtp_fu_codec = ARD_HP_VIDEO_CODEC_UNKNOWN;
  g_video.rtp_fu_hevc_hdr_off = 0;
  g_video.rtp_fu_hevc_donl_skip = 0;
  g_video.rtp_fu_next_seq = 0;
  g_video.rtp_fu_ssrc = 0;
  g_video.rtp_fu_len = 0;
}

static void ard_hp_rtp_au_reset_state(int dropped) {
  if (dropped && g_video.rtp_au_active && g_video.rtp_au_len > 0) g_video.rtp_au_dropped++;
  g_video.rtp_au_active = 0;
  g_video.rtp_au_codec = ARD_HP_VIDEO_CODEC_UNKNOWN;
  g_video.rtp_au_ts = 0;
  g_video.rtp_au_ssrc = 0;
  g_video.rtp_au_len = 0;
}

static int ard_hp_rtp_fu_append_bytes(const uint8_t *data, size_t data_len) {
  size_t need;
  uint8_t *grown;
  if (!data || data_len == 0) return 1;
  need = g_video.rtp_fu_len + data_len;
  if (need > g_video.rtp_fu_cap) {
    size_t next_cap = (g_video.rtp_fu_cap == 0) ? 4096u : g_video.rtp_fu_cap;
    while (next_cap < need) {
      if (next_cap > (SIZE_MAX / 2u)) return 0;
      next_cap *= 2u;
    }
    grown = (uint8_t *)realloc(g_video.rtp_fu_buf, next_cap);
    if (!grown) return 0;
    g_video.rtp_fu_buf = grown;
    g_video.rtp_fu_cap = next_cap;
  }
  memcpy(g_video.rtp_fu_buf + g_video.rtp_fu_len, data, data_len);
  g_video.rtp_fu_len += data_len;
  return 1;
}

static void ard_hp_adaptive_fu_reset(struct ard_hp_adaptive_fu_ctx *ctx, int dropped) {
  unsigned long i;
  if (!ctx) return;
  if (dropped && ctx->active && ctx->len > 0) g_video.rtp_fu_dropped++;
  for (i = 0; i < ARD_HP_ADAPTIVE_FU_PENDING_MAX; ++i) {
    if (ctx->pending[i].used) {
      free(ctx->pending[i].data);
      ctx->pending[i].data = NULL;
      ctx->pending[i].len = 0;
      ctx->pending[i].used = 0;
      ctx->pending[i].end = 0;
      ctx->pending[i].seq = 0;
    }
  }
  ...
  ctx->next_seq = 0;
  ctx->hevc_hdr_off = 0;
  ctx->hevc_donl_skip = 0;
  ctx->len = 0;
}

static int ard_hp_adaptive_fu_append(struct ard_hp_adaptive_fu_ctx *ctx,
                                     const uint8_t *data, size_t data_len) {
  size_t need;
  uint8_t *grown;
  if (!ctx || !data || data_len == 0) return 1;
  need = ctx->len + data_len;
  if (need > ctx->cap) {
    size_t next_cap = (ctx->cap == 0) ? 4096u : ctx->cap;
    while (next_cap < need) {
      if (next_cap > (SIZE_MAX / 2u)) return 0;
      next_cap *= 2u;
    }
    grown = (uint8_t *)realloc(ctx->buf, next_cap);
    if (!grown) return 0;
    ctx->buf = grown;
    ctx->cap = next_cap;
  }
  memcpy(ctx->buf + ctx->len, data, data_len);
  ctx->len += data_len;
  return 1;
}

static int ard_hp_rtp_seq_cmp(uint16_t a, uint16_t b) {
  return (int16_t)(a - b);
}
...
static int ard_hp_adaptive_fu_store_pending(struct ard_hp_adaptive_fu_ctx *ctx,
                                            ...
                                            int end) {
  unsigned long i;
  unsigned long free_idx = ARD_HP_ADAPTIVE_FU_PENDING_MAX;
  uint8_t *copy;
  if (!ctx || !frag || frag_len == 0) return 0;
  for (i = 0; i < ARD_HP_ADAPTIVE_FU_PENDING_MAX; ++i) {
    if (!ctx->pending[i].used) {
      if (free_idx == ARD_HP_ADAPTIVE_FU_PENDING_MAX) free_idx = i;
      continue;
    }
    if (ctx->pending[i].seq == seq) return 1;
  }
  if (free_idx == ARD_HP_ADAPTIVE_FU_PENDING_MAX) return 0;
  copy = (uint8_t *)malloc(frag_len);
  if (!copy) return 0;
  memcpy(copy, frag, frag_len);
  ctx->pending[free_idx].used = 1;
  ctx->pending[free_idx].seq = seq;
  ctx->pending[free_idx].data = copy;
  ctx->pending[free_idx].len = frag_len;
  ctx->pending[free_idx].end = end ? 1 : 0;
  return 1;
}

static int ard_hp_adaptive_fu_take_pending(struct ard_hp_adaptive_fu_ctx *ctx,
                                           uint16_t seq,
                                           uint8_t **out_frag, size_t *out_frag_len,
                                           int *out_end) {
  unsigned long i;
  if (out_frag) *out_frag = NULL;
  if (out_frag_len) *out_frag_len = 0;
  if (out_end) *out_end = 0;
  if (!ctx) return 0;
  for (i = 0; i < ARD_HP_ADAPTIVE_FU_PENDING_MAX; ++i) {
    if (!ctx->pending[i].used || ctx->pending[i].seq != seq) continue;
    if (out_frag) *out_frag = ctx->pending[i].data;
    if (out_frag_len) *out_frag_len = ctx->pending[i].len;
    if (out_end) *out_end = ctx->pending[i].end;
    ctx->pending[i].used = 0;
    ctx->pending[i].data = NULL;
    ctx->pending[i].len = 0;
    ctx->pending[i].end = 0;
...
static struct ard_hp_adaptive_fu_ctx *ard_hp_get_adaptive_fu_ctx(uint32_t ssrc, int create) {
  ...
  if (ssrc == 0u) return NULL;
  for (i = 0; i < ARD_HP_ADAPTIVE_FU_CTX_MAX; ++i) {
    struct ard_hp_adaptive_fu_ctx *ctx = &g_video.adaptive_fu[i];
    if (ctx->ssrc == ssrc) return ctx;
    if (!free_ctx && ctx->ssrc == 0u) free_ctx = ctx;
  }
  if (!create) return NULL;
  if (!free_ctx) {
    g_video.adaptive_fu_overflow++;
    return NULL;
  }
...
static int ard_hp_rtp_au_append_bytes(const uint8_t *data, size_t data_len) {
  ...
  uint8_t *grown;
  if (!data || data_len == 0) return 1;
  need = g_video.rtp_au_len + data_len;
  if (need > g_video.rtp_au_cap) {
    size_t next_cap = (g_video.rtp_au_cap == 0) ? 8192u : g_video.rtp_au_cap;
    while (next_cap < need) {
      if (next_cap > (SIZE_MAX / 2u)) return 0;
      next_cap *= 2u;
    }
    grown = (uint8_t *)realloc(g_video.rtp_au_buf, next_cap);
    if (!grown) return 0;
    g_video.rtp_au_buf = grown;
    g_video.rtp_au_cap = next_cap;
  }
  memcpy(g_video.rtp_au_buf + g_video.rtp_au_len, data, data_len);
  g_video.rtp_au_len += data_len;
  return 1;
}

static int ard_hp_rtp_au_append_nal_and_maybe_emit(const uint8_t *nal_annexb, size_t nal_annexb_len,
                                                    enum ard_hp_video_codec codec,
                                                    uint32_t ssrc, uint32_t ts, uint8_t marker,
                                                    uint8_t **out_au, size_t *out_au_len,
                                                    enum ard_hp_video_codec *out_au_codec) {
  uint8_t *emitted = NULL;

  if (out_au) *out_au = NULL;
  if (out_au_len) *out_au_len = 0;
  if (out_au_codec) *out_au_codec = ARD_HP_VIDEO_CODEC_UNKNOWN;
  if (!nal_annexb || nal_annexb_len == 0 || !out_au || !out_au_len || !out_au_codec) return -1;
  if (codec == ARD_HP_VIDEO_CODEC_UNKNOWN) return 0;

  if (!g_video.rtp_au_active) {
    g_video.rtp_au_active = 1;
    g_video.rtp_au_codec = codec;
    g_video.rtp_au_ssrc = ssrc;
    g_video.rtp_au_ts = ts;
    g_video.rtp_au_len = 0;
  } else if (g_video.rtp_au_codec != codec ||
             g_video.rtp_au_ssrc != ssrc ||
             g_video.rtp_au_ts != ts) {
    ard_hp_rtp_au_reset_state(1);
    g_video.rtp_au_active = 1;
    g_video.rtp_au_codec = codec;
    g_video.rtp_au_ssrc = ssrc;
    g_video.rtp_au_ts = ts;
  ...
  if (!ard_hp_rtp_au_append_bytes(nal_annexb, nal_annexb_len)) {
    ard_hp_rtp_au_reset_state(1);
    return -1;
  }
  if (!marker) return 0;
  if (g_video.rtp_au_len == 0) {
    ard_hp_rtp_au_reset_state(1);
    return 0;
  }

  emitted = (uint8_t *)malloc(g_video.rtp_au_len);
  if (!emitted) {
    ard_hp_rtp_au_reset_state(1);
    return -1;
  }
  memcpy(emitted, g_video.rtp_au_buf, g_video.rtp_au_len);
  *out_au = emitted;
  *out_au_len = g_video.rtp_au_len;
  *out_au_codec = (enum ard_hp_video_codec)g_video.rtp_au_codec;
  g_video.rtp_au_completed++;
  ard_hp_rtp_au_reset_state(0);
  return 1;
}

static int ard_hp_try_extract_rtp_video_bytestream(const uint8_t *packet, size_t packet_len,
                                                   uint8_t **out_data, size_t *out_len,
                                                   enum ard_hp_video_codec *out_codec,
                                                   uint32_t *out_rtp_ts, uint32_t *out_rtp_ssrc,
                                                   uint8_t *out_rtp_marker, uint8_t *out_rtp_pt) {
  ...
  if (out_codec) *out_codec = ARD_HP_VIDEO_CODEC_UNKNOWN;
  if (out_rtp_ts) *out_rtp_ts = 0;
  if (out_rtp_ssrc) *out_rtp_ssrc = 0;
  if (out_rtp_marker) *out_rtp_marker = 0;
  if (out_rtp_pt) *out_rtp_pt = 0;
  if (!packet || !out_data || !out_len || !out_codec) return 0;

  if (!ard_hp_parse_rtp_payload(packet, packet_len, &payload, &payload_len, &seq, &ssrc,
                                &timestamp, &marker, &pt)) return 0;
  if (out_rtp_ts) *out_rtp_ts = timestamp;
  if (out_rtp_ssrc) *out_rtp_ssrc = ssrc;
  if (out_rtp_marker) *out_rtp_marker = marker;
  if (out_rtp_pt) *out_rtp_pt = pt;
  if (!payload || payload_len == 0) return 0;

  /* AVConference RTP payloads prepend an 8-byte shim before the actual codec payload.
   * The inner payload then starts with regular HEVC NAL headers (e.g. 0x62/0x60/0x02). */
  if (payload_len >= 10u && payload[2] == 0x00u && payload[3] == 0x01u) {
    const uint8_t *inner = payload + 8u;
    size_t inner_len = payload_len - 8u;
    if (inner_len >= 2u) {
      uint8_t inner_type = (uint8_t)((inner[0] >> 1) & 0x3fu);
      uint8_t inner_tid = (uint8_t)(inner[1] & 0x07u);
      if (inner_tid != 0u &&
          (inner_type <= 50u || inner_type == 32u || inner_type == 33u || inner_type == 34u)) {
        payload = inner;
  ...
  if (payload_len >= 2u) {
    uint8_t hevc_type = (uint8_t)((payload[0] >> 1) & 0x3f);
    uint8_t hevc_tid = (uint8_t)(payload[1] & 0x07u);
    uint8_t hevc_layer = (uint8_t)(((payload[0] & 0x01u) << 5) | ((payload[1] >> 3) & 0x1fu));
    if (g_runtime.ard_hp_mode &&
        ...
        hevc_type <= 50u) {
      prefer_hevc = 1;
    } else if ((hevc_type == 48u || hevc_type == 49u || hevc_type == 50u) &&
               hevc_tid != 0u && hevc_layer == 0u) {
      prefer_hevc = 1;
    }
  }

  /* AVConference adaptive media uses a stable HEVC packet family on payload type 100. */
  if (payload_len >= 3u && payload[1] == 0x01u) {
    if (payload[0] == 0x60u && payload_len >= 6u) {
      size_t off = 4u;
      while (off + 2u <= payload_len) {
        uint16_t nlen = read_be_u16(payload + off);
        off += 2u;
        if (nlen == 0 || off + (size_t)nlen > payload_len) break;
        if (!ard_hp_append_annexb_nal(&buf, &len, &cap, payload + off, (size_t)nlen)) goto fail;
        off += (size_t)nlen;
      }
      if (len > 0) {
        *out_data = buf;
        *out_len = len;
        *out_codec = ARD_HP_VIDEO_CODEC_HEVC;
        return 1;
      }
    }
  }

  /* H.264 payload formats (RFC 6184). */
  if (!prefer_hevc) {
    uint8_t h264_type = (uint8_t)(payload[0] & 0x1f);
    if (h264_type >= 1 && h264_type <= 23) {
      if (!ard_hp_append_annexb_nal(&buf, &len, &cap, payload, payload_len)) goto fail;
      *out_data = buf;
      *out_len = len;
      *out_codec = ARD_HP_VIDEO_CODEC_H264;
      return 1;
    }
    if (h264_type == 24 && payload_len >= 3u) {
      size_t off = 1u;
      while (off + 2u <= payload_len) {
        uint16_t nlen = read_be_u16(payload + off);
        uint8_t inner_type;
        off += 2u;
        if (nlen == 0 || off + (size_t)nlen > payload_len) break;
        inner_type = (uint8_t)(payload[off] & 0x1f);
        if (!ard_hp_append_annexb_nal(&buf, &len, &cap, payload + off, (size_t)nlen)) goto fail;
        off += (size_t)nlen;
      }
      if (len > 0) {
    ...
    if (h264_type == 28 && payload_len >= 3u) {
      uint8_t fu_indicator = payload[0];
      uint8_t fu_header = payload[1];
      uint8_t start = (uint8_t)(fu_header & 0x80u);
      uint8_t end = (uint8_t)(fu_header & 0x40u);
      uint8_t nal_hdr = (uint8_t)((fu_indicator & 0xe0u) | (fu_header & 0x1fu));
      if (start) {
        ard_hp_rtp_fu_reset_state(0);
        g_video.rtp_fu_active = 1;
        g_video.rtp_fu_codec = ARD_HP_VIDEO_CODEC_H264;
        g_video.rtp_fu_next_seq = (uint16_t)(seq + 1u);
        g_video.rtp_fu_ssrc = ssrc;
        if (!ard_hp_rtp_fu_append_bytes((const uint8_t *)"\x00\x00\x00\x01", 4u)) goto fail;
        if (!ard_hp_rtp_fu_append_bytes(&nal_hdr, 1u)) goto fail;
        if (!ard_hp_rtp_fu_append_bytes(payload + 2u, payload_len - 2u)) goto fail;
        if (end) {
          buf = (uint8_t *)malloc(g_video.rtp_fu_len);
          if (!buf) goto fail;
          memcpy(buf, g_video.rtp_fu_buf, g_video.rtp_fu_len);
          *out_data = buf;
          *out_len = g_video.rtp_fu_len;
          *out_codec = ARD_HP_VIDEO_CODEC_H264;
          g_video.rtp_fu_completed++;
          ard_hp_rtp_fu_reset_state(0);
          return 1;
        }
        return 0;
      }
      if (!g_video.rtp_fu_active || g_video.rtp_fu_codec != ARD_HP_VIDEO_CODEC_H264 ||
          g_video.rtp_fu_ssrc != ssrc || g_video.rtp_fu_next_seq != seq) {
        ard_hp_rtp_fu_reset_state(1);
        return 0;
      }
      g_video.rtp_fu_next_seq = (uint16_t)(seq + 1u);
      if (!ard_hp_rtp_fu_append_bytes(payload + 2u, payload_len - 2u)) goto fail;
  ...
  if (payload_len >= 2u) {
    ...
    if (nalu_type <= 47u) {
      if (payload_len >= 4u && payload[1] == 0x01u) {
        ...
        if (len + (payload_len - 4u) > cap) {
          size_t need = len + (payload_len - 4u);
          size_t next_cap = (cap == 0) ? 1024u : cap;
          uint8_t *grown;
          while (next_cap < need) {
            if (next_cap > (SIZE_MAX / 2u)) goto fail;
            next_cap *= 2u;
          }
          grown = (uint8_t *)realloc(buf, next_cap);
          if (!grown) goto fail;
        ...
        memcpy(buf + len, payload + 4u, payload_len - 4u);
        len += payload_len - 4u;
      } else {
        if (!ard_hp_append_annexb_nal(&buf, &len, &cap, payload, payload_len)) goto fail;
      }
    ...
    if (nalu_type == 48u && payload_len >= 4u) {
      size_t ap_base;
      for (ap_base = 2u; ap_base <= 4u; ap_base += 2u) {
        size_t off = ap_base;
        size_t trial_len = 0;
        size_t trial_cap = 0;
        uint8_t *trial_buf = NULL;
        while (off + 2u <= payload_len) {
          uint16_t nlen = read_be_u16(payload + off);
          off += 2u;
          if (nlen == 0 || off + (size_t)nlen > payload_len) break;
          if (!ard_hp_append_annexb_nal(&trial_buf, &trial_len, &trial_cap, payload + off, (size_t)nlen)) {
            free(trial_buf);
            goto fail;
          }
          off += (size_t)nlen;
    ...
    if (nalu_type == 49u && payload_len >= 4u) {
      /* Adaptive AVConference RTP profile: HEVC FU header is at payload[2] and
       * a 2-byte DONL follows at payload[3..4] when tid==1 profile is used. */
      if (payload[1] == 0x01u) {
        size_t fu_header_off = 2u;
        size_t donl_skip = 2u;
        size_t frag_off = fu_header_off + 1u + donl_skip;
        uint8_t fu_header;
        uint8_t start;
        uint8_t end;
        uint8_t fu_type;
        ...
        int pending_end = 0;
        if (payload_len <= frag_off) return 0;
        ...
        if (start) {
          ...
          fu_ctx->active = 1;
          fu_ctx->hevc_hdr_off = (uint8_t)fu_header_off;
          fu_ctx->hevc_donl_skip = (uint8_t)donl_skip;
          fu_ctx->next_seq = (uint16_t)(seq + 1u);
          if (!ard_hp_adaptive_fu_append(fu_ctx, (const uint8_t *)"\x00\x00\x00\x01", 4u)) goto fail;
          if (!ard_hp_adaptive_fu_append(fu_ctx, nal_hdr, 2u)) goto fail;
          if (!ard_hp_adaptive_fu_append(fu_ctx, payload + frag_off, payload_len - frag_off)) goto fail;
          if (end) {
            buf = (uint8_t *)malloc(fu_ctx->len);
            if (!buf) goto fail;
            memcpy(buf, fu_ctx->buf, fu_ctx->len);
            *out_data = buf;
            *out_len = fu_ctx->len;
            *out_codec = ARD_HP_VIDEO_CODEC_HEVC;
            g_video.rtp_fu_completed++;
            ard_hp_adaptive_fu_reset(fu_ctx, 0);
            return 1;
          }
          return 0;
        }
        ...
        seq_order = ard_hp_rtp_seq_cmp(seq, fu_ctx->next_seq);
        if (seq_order < 0) return 0;
        if (seq_order > 0) {
          uint16_t seq_gap = (uint16_t)(seq - fu_ctx->next_seq);
          if (seq_gap > fu_gap_max) {
            ard_hp_adaptive_fu_reset(fu_ctx, 1);
            return 0;
          }
          if (!ard_hp_adaptive_fu_store_pending(fu_ctx, seq, payload + frag_off, payload_len - frag_off, end)) {
            ard_hp_adaptive_fu_reset(fu_ctx, 1);
          }
          return 0;
        }
        if (!ard_hp_adaptive_fu_append(fu_ctx, payload + frag_off, payload_len - frag_off)) {
          ard_hp_adaptive_fu_reset(fu_ctx, 1);
          return 0;
        }
        fu_ctx->next_seq = (uint16_t)(seq + 1u);
        ...
        while (ard_hp_adaptive_fu_take_pending(fu_ctx, fu_ctx->next_seq,
                                               &pending_frag, &pending_len, &pending_end)) {
          if (!ard_hp_adaptive_fu_append(fu_ctx, pending_frag, pending_len)) {
            free(pending_frag);
            ard_hp_adaptive_fu_reset(fu_ctx, 1);
            return 0;
          }
          ...
          pending_frag = NULL;
          pending_len = 0;
          fu_ctx->next_seq = (uint16_t)(fu_ctx->next_seq + 1u);
          if (pending_end) {
            buf = (uint8_t *)malloc(fu_ctx->len);
            if (!buf) goto fail;
            memcpy(buf, fu_ctx->buf, fu_ctx->len);
      ...
      /* Fallback RFC7798 FU handling for non-adaptive profiles. */
      {
        size_t fu_header_off;
        for (fu_header_off = 2u; fu_header_off <= 5u; ++fu_header_off) {
          size_t donl_skip;
          for (donl_skip = 0u; donl_skip <= 2u; donl_skip += 2u) {
            size_t frag_off = fu_header_off + 1u + donl_skip;
            uint8_t fu_header;
            uint8_t start;
            uint8_t end;
            ...
            uint8_t nal_hdr[2];
            int seq_match;
            if (payload_len <= frag_off) continue;
            fu_header = payload[fu_header_off];
            start = (uint8_t)(fu_header & 0x80u);
            end = (uint8_t)(fu_header & 0x40u);
            fu_type = (uint8_t)(fu_header & 0x3fu);
            if (fu_type == 0u) continue;
            seq_match = (g_video.rtp_fu_active &&
                         g_video.rtp_fu_codec == ARD_HP_VIDEO_CODEC_HEVC &&
                         g_video.rtp_fu_ssrc == ssrc &&
                         g_video.rtp_fu_next_seq == seq &&
                         g_video.rtp_fu_hevc_hdr_off == (uint8_t)fu_header_off &&
                         g_video.rtp_fu_hevc_donl_skip == (uint8_t)donl_skip);
            if (!start && !seq_match) continue;
            nal_hdr[0] = (uint8_t)((payload[0] & 0x81u) | (fu_type << 1));
            nal_hdr[1] = payload[1];
            if (start) {
              ard_hp_rtp_fu_reset_state(0);
              ...
              g_video.rtp_fu_ssrc = ssrc;
              if (!ard_hp_rtp_fu_append_bytes((const uint8_t *)"\x00\x00\x00\x01", 4u)) goto fail;
              if (!ard_hp_rtp_fu_append_bytes(nal_hdr, 2u)) goto fail;
              if (!ard_hp_rtp_fu_append_bytes(payload + frag_off, payload_len - frag_off)) goto fail;
              if (end) {
                buf = (uint8_t *)malloc(g_video.rtp_fu_len);
                if (!buf) goto fail;
                memcpy(buf, g_video.rtp_fu_buf, g_video.rtp_fu_len);
      ...
      if (g_video.rtp_fu_active && g_video.rtp_fu_codec == ARD_HP_VIDEO_CODEC_HEVC &&
          g_video.rtp_fu_ssrc == ssrc && g_video.rtp_fu_next_seq == seq) {
        ard_hp_rtp_fu_reset_state(1);
      }
      return 0;
...
static int ard_hp_hex_nibble(int c) {
...
static int ard_hp_decode_hex_blob(const char *hex, uint8_t **out, size_t *out_len) {
  size_t cap = 0;
  size_t len = 0;
  uint8_t *buf = NULL;
  int have_hi = 0;
  ...
  for (p = (const unsigned char *)hex; *p != '\0'; ++p) {
    int v;
    if (*p == ' ' || *p == ':' || *p == '-' || *p == '\t' || *p == '\n' || *p == '\r') continue;
    v = ard_hp_hex_nibble((int)*p);
    if (v < 0) {
      free(buf);
      return 0;
    }
    ...
    if (len >= cap) {
      size_t next_cap = (cap == 0) ? 256u : cap * 2u;
...
static int ard_hp_parse_hex_bytes(const char *hex, uint8_t *out, size_t out_len) {
  size_t out_off = 0;
  int have_hi = 0;
  int hi = 0;
  const unsigned char *p;
  ...
  for (p = (const unsigned char *)hex; *p != '\0'; ++p) {
    ...
    if (out_off >= out_len) return 0;
    out[out_off++] = (uint8_t)((hi << 4) | v);
    have_hi = 0;
  }
  return (have_hi == 0 && out_off == out_len) ? 1 : 0;
}

static void ard_hp_shutdown_srtp_contexts(void) {
  unsigned long i;
  for (i = 0; i < g_video.srtp_ctx_count; ++i) {
    if (g_video.srtp_ctx[i]) {
      rfbClientARDHPSRTPDestroy(g_video.srtp_ctx[i]);
      g_video.srtp_ctx[i] = NULL;
    }
  }
  g_video.srtp_ctx_count = 0;
  for (i = 0; i < g_video.srtp_tx_ctx_count; ++i) {
    if (g_video.srtp_tx_ctx[i]) {
      rfbClientARDHPSRTPDestroy(g_video.srtp_tx_ctx[i]);
      g_video.srtp_tx_ctx[i] = NULL;
    }
  }
  g_video.srtp_tx_ctx_count = 0;
  g_video.srtp_last_rx_ctx_idx = 0;
  g_video.srtp_last_rx_ctx_valid = 0;
  g_video.srtcp_preferred_tx_ctx_idx = 0;
  g_video.srtcp_preferred_tx_ctx_valid = 0;
}

static void ard_hp_try_add_srtp_suite5_key(const uint8_t key_bytes[46], const char *origin) {
  rfbARDHPSRTPContext *ctx80;
  rfbARDHPSRTPContext *ctx32;
  rfbARDHPSRTPContext *tx80;
  rfbARDHPSRTPContext *tx32;

  if (!key_bytes || g_video.srtp_ctx_count >= ARD_HP_SRTP_CTX_MAX) return;

  ctx80 = rfbClientARDHPSRTPCreateInboundSuite5(key_bytes, 46, FALSE);
  if (ctx80 && g_video.srtp_ctx_count < ARD_HP_SRTP_CTX_MAX) {
    g_video.srtp_ctx[g_video.srtp_ctx_count++] = ctx80;
  }
  ctx32 = rfbClientARDHPSRTPCreateInboundSuite5(key_bytes, 46, TRUE);
  if (ctx32 && g_video.srtp_ctx_count < ARD_HP_SRTP_CTX_MAX) {
    g_video.srtp_ctx[g_video.srtp_ctx_count++] = ctx32;
  } else if (ctx32) {
    rfbClientARDHPSRTPDestroy(ctx32);
  }

  tx80 = rfbClientARDHPSRTPCreateOutboundSuite5(key_bytes, 46, FALSE);
  if (tx80 && g_video.srtp_tx_ctx_count < ARD_HP_SRTP_CTX_MAX) {
    g_video.srtp_tx_ctx[g_video.srtp_tx_ctx_count++] = tx80;
  } else if (tx80) {
    rfbClientARDHPSRTPDestroy(tx80);
  }
  tx32 = rfbClientARDHPSRTPCreateOutboundSuite5(key_bytes, 46, TRUE);
  if (tx32 && g_video.srtp_tx_ctx_count < ARD_HP_SRTP_CTX_MAX) {
    g_video.srtp_tx_ctx[g_video.srtp_tx_ctx_count++] = tx32;
  } else if (tx32) {
    rfbClientARDHPSRTPDestroy(tx32);
  }

  (void)origin;
}

static void ard_hp_try_add_srtp_keys_from_media_options_hex(const char *media_options_hex) {
  uint8_t *msg = NULL;
  size_t msg_len = 0;
  uint16_t audio_offer_len;
  uint16_t video1_offer_len;
  size_t keys_off_audio;
  size_t keys_off_video1;
  size_t keys_span = 92u;

  if (!media_options_hex || !*media_options_hex) return;
  if (!ard_hp_decode_hex_blob(media_options_hex, &msg, &msg_len)) return;
  if (!msg || msg_len < 36u || msg[0] != 0x1c) {
    free(msg);
    return;
  }

  audio_offer_len = read_be_u16(msg + 10u);
  video1_offer_len = read_be_u16(msg + 12u);
  keys_off_audio = 36u;
  keys_off_video1 = keys_off_audio + keys_span + (size_t)audio_offer_len;
  if (keys_off_video1 + keys_span > msg_len) {
    free(msg);
    return;
  }

  /* Server->client video stream usually uses the video offer key pair first. */
  ard_hp_try_add_srtp_suite5_key(msg + keys_off_video1, "0x1c.video1.key1");
  ard_hp_try_add_srtp_suite5_key(msg + keys_off_video1 + 46u, "0x1c.video1.key2");

  /* Keep audio keys as fallback candidates for unknown stream/key direction mappings. */
  ard_hp_try_add_srtp_suite5_key(msg + keys_off_audio, "0x1c.audio.key1");
  ard_hp_try_add_srtp_suite5_key(msg + keys_off_audio + 46u, "0x1c.audio.key2");

  if (video1_offer_len == 0) {
    rfbClientLog("ard-hp: MediaStreamOptions 0x1c has zero video1 offer length\n");
  }
  free(msg);
}

static void ard_hp_init_video_srtp_contexts(void) {
  const char *key_hex_1;
  const char *key_hex_2;
  const char *key_hex_3;
  const char *media_options_hex;
  const char *keys[3];
  int idx;

  if (g_video.srtp_init_attempted &&
      (g_video.srtp_ctx_count > 0 || g_video.srtp_tx_ctx_count > 0)) {
    return;
  }
  if (g_video.srtp_init_attempted &&
      g_video.srtp_ctx_count == 0 &&
      g_video.srtp_tx_ctx_count == 0) {
    g_video.srtp_init_attempted = 0;
  }
  if (g_video.srtp_init_attempted) return;
  g_video.srtp_init_attempted = 1;

  key_hex_1 = getenv("VNC_ARD_HP_MEDIA_SRTP_KEY_HEX");
  key_hex_2 = getenv("VNC_ARD_HP_MEDIA_SRTP_KEY2_HEX");
  key_hex_3 = getenv("VNC_ARD_HP_MEDIA_SRTP_KEY3_HEX");
  keys[0] = key_hex_1;
  keys[1] = key_hex_2;
  keys[2] = key_hex_3;

  for (idx = 0; idx < 3 && g_video.srtp_ctx_count < ARD_HP_SRTP_CTX_MAX; ++idx) {
    uint8_t key_bytes[46];
    if (!keys[idx] || !*keys[idx]) continue;
    if (!ard_hp_parse_hex_bytes(keys[idx], key_bytes, sizeof(key_bytes))) {
      rfbClientErr("ard-hp: invalid SRTP key hex in VNC_ARD_HP_MEDIA_SRTP_KEY%s_HEX (need 46 bytes)\n",
                   idx == 0 ? "" : (idx == 1 ? "2" : "3"));
      continue;
    }
    ard_hp_try_add_srtp_suite5_key(key_bytes, idx == 0 ? "env.key1" : (idx == 1 ? "env.key2" : "env.key3"));
  }

  media_options_hex = getenv("VNC_ARD_HP_MEDIA_STREAM_OPTIONS_HEX");
  if ((!media_options_hex || !*media_options_hex) && g_last_media_options_hex && *g_last_media_options_hex) {
    media_options_hex = g_last_media_options_hex;
  }
  if (g_video.srtp_ctx_count < ARD_HP_SRTP_CTX_MAX) {
    ard_hp_try_add_srtp_keys_from_media_options_hex(media_options_hex);
  }

  if (g_video.srtp_ctx_count > 0 && !g_video.srtp_contexts_logged) {
    g_video.srtp_contexts_logged = 1;
    rfbClientLog("ard-hp: SRTP suite5 contexts ready inbound=%lu outbound=%lu\n",
                 g_video.srtp_ctx_count, g_video.srtp_tx_ctx_count);
  }
}

static int ard_hp_try_unprotect_video_packet(const uint8_t *packet, size_t packet_len,
                                             uint8_t **out_packet, size_t *out_len) {
  unsigned long i;
  if (out_packet) *out_packet = NULL;
  if (out_len) *out_len = 0;
  if (!packet || packet_len == 0 || !out_packet || !out_len) return 0;

  ard_hp_init_video_srtp_contexts();
  if (g_video.srtp_ctx_count == 0) return 0;

  for (i = 0; i < g_video.srtp_ctx_count; ++i) {
    if (!g_video.srtp_ctx[i]) continue;
    if (rfbClientARDHPSRTPUnprotectPacket(g_video.srtp_ctx[i], packet, packet_len, out_packet, out_len)) {
      g_video.srtp_unprotect_ok++;
      g_video.srtp_last_rx_ctx_idx = i;
      g_video.srtp_last_rx_ctx_valid = 1;
      if (g_video.srtp_tx_ctx_count > 0) {
        unsigned long preferred = (i < g_video.srtp_tx_ctx_count) ? i : (i % g_video.srtp_tx_ctx_count);
        if (!g_video.srtcp_preferred_tx_ctx_valid || g_video.srtcp_preferred_tx_ctx_idx != preferred) {
          g_video.srtcp_preferred_tx_ctx_idx = preferred;
          g_video.srtcp_preferred_tx_ctx_valid = 1;
          rfbClientLog("ard-hp: selected outbound SRTP context index=%lu from inbound media context index=%lu\n",
                       preferred, i);
        }
      }
      return 1;
  ...
  g_video.srtp_unprotect_fail++;
  return 0;
}

static int ard_hp_ensure_video_decoder(enum ard_hp_video_codec codec_kind) {
#if defined(__APPLE__) && defined(ARDHPDEBUG_HAS_VIDEOTOOLBOX)
  if (ard_hp_should_use_videotoolbox(codec_kind)) {
    if (g_video.backend == ARD_HP_DECODER_BACKEND_VIDEOTOOLBOX &&
        g_video.codec == (int)codec_kind) {
      return 1;
    }
    ard_hp_shutdown_video_decoder();
    g_video.backend = ARD_HP_DECODER_BACKEND_VIDEOTOOLBOX;
    g_video.codec = (int)codec_kind;
    return 1;
  }
#endif
  ...
  if (codec_kind == ARD_HP_VIDEO_CODEC_UNKNOWN) return 0;
  if (g_video.backend == ARD_HP_DECODER_BACKEND_FFMPEG &&
      g_video.codec_ctx && g_video.frame && g_video.packet &&
      g_video.codec == (int)codec_kind) return 1;

  ard_hp_shutdown_video_decoder();

  codec_id = ard_hp_video_codec_to_ffmpeg_id(codec_kind);
  codec_name = ard_hp_video_codec_name(codec_kind);
  decoder = avcodec_find_decoder(codec_id);
  if (!decoder) {
    rfbClientErr("ard-hp: ffmpeg %s decoder not found\n", codec_name);
    return 0;
  }
  ...
  if (ret < 0) {
    char prefix[96];
    snprintf(prefix, sizeof(prefix), "avcodec_open2(%s) failed", codec_name);
    ard_hp_log_av_error(prefix, ret);
    avcodec_free_context(&g_video.codec_ctx);
    return 0;
  }
  ...
  g_video.backend = ARD_HP_DECODER_BACKEND_FFMPEG;
  g_video.codec = (int)codec_kind;
  return 1;
}

static void ard_hp_release_lane_decoder(struct ard_hp_lane_decoder *lane) {
  if (!lane) return;
  lane->ssrc = 0;
  lane->codec = ARD_HP_VIDEO_CODEC_UNKNOWN;
  if (lane->packet) {
    av_packet_free(&lane->packet);
  }
  if (lane->frame) {
    av_frame_free(&lane->frame);
  }
  if (lane->codec_ctx) {
    avcodec_free_context(&lane->codec_ctx);
  }
}

static struct ard_hp_lane_decoder *ard_hp_get_or_create_lane_decoder(uint32_t ssrc,
                                                                     enum ard_hp_video_codec codec_kind) {
  unsigned long i;
  struct ard_hp_lane_decoder *free_lane = NULL;
  const AVCodec *decoder;
  enum AVCodecID codec_id;

  if (ssrc == 0 || codec_kind == ARD_HP_VIDEO_CODEC_UNKNOWN) return NULL;
  for (i = 0; i < ARD_HP_ADAPTIVE_LANE_DECODER_MAX; ++i) {
    struct ard_hp_lane_decoder *lane = &g_video.lane_decoder[i];
    if (lane->ssrc == ssrc && lane->codec == (int)codec_kind && lane->codec_ctx && lane->frame && lane->packet) {
      return lane;
    }
    if (!free_lane && lane->ssrc == 0) free_lane = lane;
  }
  if (!free_lane) {
    g_video.lane_decoder_overflow++;
    return NULL;
  }
  ...
  free_lane->codec_ctx = avcodec_alloc_context3(decoder);
  if (!free_lane->codec_ctx) {
  ...
  free_lane->frame = av_frame_alloc();
  free_lane->packet = av_packet_alloc();
  if (!free_lane->frame || !free_lane->packet) {
    ard_hp_release_lane_decoder(free_lane);
    return NULL;
  }
  free_lane->ssrc = ssrc;
  free_lane->codec = (int)codec_kind;
  rfbClientLog("ard-hp: adaptive lane decoder created ssrc=0x%08x codec=%s\n",
               (unsigned)ssrc, ard_hp_video_codec_name(codec_kind));
  return free_lane;
}

static int ard_hp_decode_with_lane_decoder(rfbClient *client,
                                           struct ard_hp_lane_decoder *lane,
                                           const uint8_t *bytestream, size_t bytestream_len,
                                           uint32_t rtp_ssrc, int adaptive_strip_mode,
                                           int *out_decoded_frames) {
  int ret;
  int decoded_frames = 0;
  if (out_decoded_frames) *out_decoded_frames = 0;
  if (!client || !lane || !lane->codec_ctx || !lane->packet || !lane->frame ||
      !bytestream || bytestream_len == 0) {
    return 0;
  }

  ret = av_new_packet(lane->packet, (int)bytestream_len);
  if (ret < 0) return 0;
  memcpy(lane->packet->data, bytestream, bytestream_len);
  ret = avcodec_send_packet(lane->codec_ctx, lane->packet);
  av_packet_unref(lane->packet);
  if (ret < 0 && ret != AVERROR(EAGAIN)) {
    return 0;
  }

  for (;;) {
    ret = avcodec_receive_frame(lane->codec_ctx, lane->frame);
    if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) break;
    if (ret < 0) return 0;
    if (ard_hp_push_decoded_frame(client, lane->frame, rtp_ssrc, adaptive_strip_mode)) {
      decoded_frames++;
    }
    av_frame_unref(lane->frame);
  }

  if (out_decoded_frames) *out_decoded_frames = decoded_frames;
  return 1;
}

static void ard_hp_flush_video_decoder_state(void) {
#if defined(ARDHPDEBUG_HAS_FFMPEG)
  unsigned long i;
  if (g_video.codec_ctx) avcodec_flush_buffers(g_video.codec_ctx);
  for (i = 0; i < ARD_HP_ADAPTIVE_LANE_DECODER_MAX; ++i) {
    if (g_video.lane_decoder[i].codec_ctx) {
      avcodec_flush_buffers(g_video.lane_decoder[i].codec_ctx);
    }
  }
  ard_hp_rtp_fu_reset_state(1);
  for (i = 0; i < ARD_HP_ADAPTIVE_FU_CTX_MAX; ++i) {
    ard_hp_adaptive_fu_reset(&g_video.adaptive_fu[i], 1);
  }
  ard_hp_rtp_au_reset_state(1);
#endif
}

static int ard_hp_find_or_add_adaptive_lane(uint32_t ssrc) {
  unsigned long i;
  unsigned long ins;
  if (ssrc == 0) return 0;
  for (i = 0; i < g_video.adaptive_lane_count; ++i) {
    if (g_video.adaptive_lane_ssrc[i] == ssrc) return (int)i;
  }
  if (g_video.adaptive_lane_count >= ARD_HP_RTP_SSRC_TRACK_MAX) return (int)(ARD_HP_RTP_SSRC_TRACK_MAX - 1u);
  ins = g_video.adaptive_lane_count;
  for (i = 0; i < g_video.adaptive_lane_count; ++i) {
    if (ssrc < g_video.adaptive_lane_ssrc[i]) {
      ins = i;
      break;
    }
  }
  for (i = g_video.adaptive_lane_count; i > ins; --i) {
    g_video.adaptive_lane_ssrc[i] = g_video.adaptive_lane_ssrc[i - 1u];
  }
  g_video.adaptive_lane_ssrc[ins] = ssrc;
  g_video.adaptive_lane_count++;
  rfbClientLog("ard-hp: adaptive lane[%lu] <= SSRC 0x%08x (ordered)\n", ins, (unsigned)ssrc);
  return (int)ins;
}

static int ard_hp_push_decoded_strip_frame(rfbClient *client, AVFrame *frame, uint32_t rtp_ssrc) {
  int src_w;
  int src_h;
  int dst_w;
  int dst_h;
  ...
  if (!client || !frame) return 0;
  src_w = frame->width;
  src_h = frame->height;
  if (src_w <= 0 || src_h <= 0) return 0;

  if ((ard_hp_backing_width(client) > 0 && ard_hp_backing_height(client) > 0) &&
      ((uint16_t)client->width != ard_hp_backing_width(client) ||
       (uint16_t)client->height != ard_hp_backing_height(client))) {
    if (!ard_hp_resize_framebuffer_if_needed(client, ard_hp_backing_width(client), ard_hp_backing_height(client))) {
      return 0;
    }
  }
  ...
  dst_w = client->width;
  dst_h = client->height;
  if (!client->frameBuffer || dst_w <= 0 || dst_h <= 0) return 0;
  if (src_h >= dst_h) return 0;

  if (!g_video.strip_sws || g_video.strip_sws_src_w != src_w || g_video.strip_sws_src_h != src_h) {
    sws_freeContext(g_video.strip_sws);
    g_video.strip_sws = sws_getContext(src_w, src_h, (enum AVPixelFormat)frame->format,
                                       src_w, src_h, AV_PIX_FMT_BGRA,
                                       SWS_BILINEAR, NULL, NULL, NULL);
    if (!g_video.strip_sws) return 0;
    g_video.strip_sws_src_w = src_w;
    g_video.strip_sws_src_h = src_h;
  }

  need = (size_t)src_w * 4u * (size_t)src_h;
  if (need > g_video.strip_bgra_cap) {
    uint8_t *grown = (uint8_t *)realloc(g_video.strip_bgra_buf, need);
    if (!grown) return 0;
    g_video.strip_bgra_buf = grown;
    g_video.strip_bgra_cap = need;
  }

  out_data[0] = g_video.strip_bgra_buf;
  out_data[1] = NULL;
  out_data[2] = NULL;
  out_data[3] = NULL;
  out_linesize[0] = src_w * 4;
  out_linesize[1] = 0;
  out_linesize[2] = 0;
  out_linesize[3] = 0;
  if (sws_scale(g_video.strip_sws, (const uint8_t *const *)frame->data, frame->linesize, 0, src_h,
                out_data, out_linesize) <= 0) {
    return 0;
  }

  lane_idx = ard_hp_find_or_add_adaptive_lane(rtp_ssrc);
  lane_y = lane_idx * src_h;
  if (lane_y >= dst_h) lane_y = (lane_idx % 2) * src_h;
  if (lane_y >= dst_h) return 0;

  copy_h = src_h;
  if (lane_y + copy_h > dst_h) copy_h = dst_h - lane_y;
  if (copy_h <= 0) return 0;
  copy_w = src_w < dst_w ? src_w : dst_w;
  if (copy_w <= 0) return 0;
  row_bytes = (size_t)copy_w * 4u;
  dst_stride = dst_w * 4;

  for (y = 0; y < copy_h; ++y) {
    uint8_t *dst_row = client->frameBuffer + (size_t)(lane_y + y) * (size_t)dst_stride;
    const uint8_t *src_row = g_video.strip_bgra_buf + (size_t)y * (size_t)out_linesize[0];
    memcpy(dst_row, src_row, row_bytes);
  }

  g_video.frames_decoded++;
  g_video.strip_frames_composited++;
  g_video.strip_last_ssrc = rtp_ssrc;
  on_fb_update(client, 0, lane_y, copy_w, copy_h);
  on_fb_update_done(client);
  return 1;
}
...
static int ard_hp_push_decoded_frame(rfbClient *client, AVFrame *frame,
  ...
  if (!client || !frame || !client->frameBuffer) return 0;
  ...
  if (dst_w <= 0 || dst_h <= 0 || !client->frameBuffer) return 0;

  dst_stride = dst_w * 4;
  dst_data[0] = client->frameBuffer;
  dst_data[1] = NULL;
  dst_data[2] = NULL;
  dst_data[3] = NULL;
  dst_linesize[0] = dst_stride;
  dst_linesize[1] = 0;
  dst_linesize[2] = 0;
  dst_linesize[3] = 0;

  if (!g_video.sws || g_video.sws_src_w != src_w || g_video.sws_src_h != src_h || g_video.sws_dst_w != dst_w ||
      g_video.sws_dst_h != dst_h) {
    sws_freeContext(g_video.sws);
    g_video.sws = sws_getContext(src_w, src_h, (enum AVPixelFormat)frame->format, dst_w, dst_h, AV_PIX_FMT_BGRA,
                                SWS_BILINEAR, NULL, NULL, NULL);
    if (!g_video.sws) return 0;
    g_video.sws_src_w = src_w;
    g_video.sws_src_h = src_h;
    g_video.sws_dst_w = dst_w;
    g_video.sws_dst_h = dst_h;
  }

  ret = sws_scale(g_video.sws, (const uint8_t *const *)frame->data, frame->linesize, 0, src_h, dst_data, dst_linesize);
  if (ret <= 0) return 0;

  g_video.frames_decoded++;
  on_fb_update(client, 0, 0, dst_w, dst_h);
  on_fb_update_done(client);
  return 1;
}

static int ard_hp_decode_video_media_packet(rfbClient *client, uint32_t encoding, const uint8_t *payload, size_t total_len) {
  uint8_t *decrypted_packet = NULL;
  size_t decrypted_len = 0;
  int srtp_unprotected = 0;
  uint8_t *bytestream = NULL;
  size_t bytestream_len = 0;
  const uint8_t *video_packet = payload;
  size_t video_packet_len = total_len;
  rfbBool from_length_prefix = FALSE;
  enum ard_hp_video_codec hinted_codec = ARD_HP_VIDEO_CODEC_UNKNOWN;
  enum ard_hp_video_codec assembled_codec = ARD_HP_VIDEO_CODEC_UNKNOWN;
  enum ard_hp_video_codec active_codec;
  enum ard_hp_video_codec fallback_codec;
  const char *input_kind = "annexb";
  int rtp_input = 0;
  uint32_t rtp_ts = 0;
  uint32_t rtp_ssrc = 0;
  uint8_t rtp_marker = 0;
  uint8_t rtp_pt = 0;
  uint8_t *assembled_au = NULL;
  size_t assembled_au_len = 0;
  int avc_multistream_profile = 0;
  int adaptive_strip_mode = 0;
  int allow_fallback_switch = 1;
  int use_lane_decoder = 0;
  int lane_decoders_requested = 0;
  int ret;
  int decoded_frames = 0;
  const uint8_t *parse_in;
  int parse_len;
  int use_parser;
  struct ard_hp_lane_decoder *lane_decoder = NULL;

  if (!client || !payload || total_len == 0) return 1;
  if (encoding != 0x3f3 && encoding != 0x3ea) return 1;
  ard_hp_media_dump_init();

  if (ard_hp_try_unprotect_video_packet(payload, total_len, &decrypted_packet, &decrypted_len)) {
    srtp_unprotected = 1;
    video_packet = decrypted_packet;
    video_packet_len = decrypted_len;
    if (g_media_dump_enabled) {
      ard_hp_media_dump_write("pkt_decrypted_all", g_media_dump_decrypted_all_index++,
                              video_packet, video_packet_len);
    }
    /* UDP adaptive media packets are SRTP/RTP; prefer RTP extraction first.
     * Falling back to generic bytestream extraction here can misclassify RTP payloads. */
    if (ard_hp_try_extract_rtp_video_bytestream(video_packet, video_packet_len, &bytestream, &bytestream_len,
                                                &hinted_codec, &rtp_ts, &rtp_ssrc, &rtp_marker, &rtp_pt)) {
      if (g_media_dump_enabled) {
        ard_hp_media_dump_write("pkt_decrypted", g_media_dump_packet_index++, video_packet, video_packet_len);
      }
      input_kind = "srtp-rtp";
      rtp_input = 1;
    }
  ...
  /* For SRTP/UDP packets, never attempt non-RTP bytestream fallback on the raw
   * encrypted packet. FU continuation packets legitimately yield no AU yet. */
  if (srtp_unprotected && !bytestream) {
    free(decrypted_packet);
    return 1;
  }

  if (!bytestream &&
      rfbClientARDHPExtractVideoBytestream(encoding, payload, total_len, &bytestream, &bytestream_len,
                                           &from_length_prefix)) {
    input_kind = from_length_prefix ? "length-prefixed" : "annexb";
  }

  if (!bytestream) {
    if (env_flag_enabled("VNC_ARD_HP_LOG_RTP_PAYLOAD") && g_video.rtp_probe_logged < 24u) {
      const uint8_t *rtp_payload = NULL;
      size_t rtp_payload_len = 0;
      uint16_t rtp_seq = 0;
      uint32_t rtp_ssrc_probe = 0;
      char prefix[65];
      uint32_t rtp_ts_probe = 0;
      uint8_t rtp_marker_probe = 0;
      uint8_t rtp_pt_probe = 0;
      if (ard_hp_parse_rtp_payload(payload, total_len, &rtp_payload, &rtp_payload_len, &rtp_seq, &rtp_ssrc_probe,
                                   &rtp_ts_probe, &rtp_marker_probe, &rtp_pt_probe)) {
        size_t n = rtp_payload_len > 32u ? 32u : rtp_payload_len;
        ard_hp_hex_encode(rtp_payload, n, prefix, sizeof(prefix));
        rfbClientLog("ard-hp: rtp probe pt=%u marker=%u seq=%u ts=%u ssrc=0x%08x payload_len=%lu prefix=%s\n",
                     (unsigned)rtp_pt_probe, (unsigned)rtp_marker_probe, (unsigned)rtp_seq,
                     (unsigned)rtp_ts_probe, (unsigned)rtp_ssrc_probe, (unsigned long)rtp_payload_len, prefix);
      }
      g_video.rtp_probe_logged++;
    }
    if (!ard_hp_try_extract_rtp_video_bytestream(payload, total_len, &bytestream, &bytestream_len,
                                                 &hinted_codec, &rtp_ts, &rtp_ssrc, &rtp_marker, &rtp_pt)) {
      free(decrypted_packet);
      return 1;
    }
  ...
  if (!bytestream || bytestream_len == 0) {
    free(bytestream);
    return 1;
  }
  if (rtp_input) {
    avc_multistream_profile = (hinted_codec == ARD_HP_VIDEO_CODEC_HEVC &&
                               rtp_pt == 100u &&
                               g_runtime.ard_hp_mode &&
                               env_flag_enabled("VNC_ARD_HP_ADAPTIVE_MODE") &&
                               bytestream_len >= 4u &&
                               bytestream[0] == 0x00u &&
                               bytestream[1] == 0x00u &&
                               bytestream[2] == 0x00u &&
                               bytestream[3] == 0x01u);
    ard_hp_note_rtp_ssrc_packet(rtp_ssrc, hinted_codec, bytestream, bytestream_len);
    if (hinted_codec == ARD_HP_VIDEO_CODEC_HEVC) {
      ard_hp_update_param_sets_from_annexb(hinted_codec, bytestream, bytestream_len);
    }
    if (!avc_multistream_profile && !ard_hp_should_accept_rtp_ssrc(rtp_ssrc)) {
      free(bytestream);
      return 1;
    }
    if (avc_multistream_profile &&
        env_flag_enabled("VNC_ARD_HP_ADAPTIVE_SINGLE_SSRC_FALLBACK") &&
        g_video.rtp_selected_ssrc_valid &&
        rtp_ssrc != g_video.rtp_selected_ssrc) {
      free(bytestream);
      return 1;
    }
    if (!avc_multistream_profile) {
      int emit_rc = ard_hp_rtp_au_append_nal_and_maybe_emit(bytestream, bytestream_len, hinted_codec,
                                                             rtp_ssrc, rtp_ts, rtp_marker,
                                                             &assembled_au, &assembled_au_len, &assembled_codec);
      free(bytestream);
      bytestream = NULL;
      if (emit_rc < 0) return 1;
      if (emit_rc == 0) return 1;
      ard_hp_note_rtp_ssrc_au(rtp_ssrc);
      bytestream = assembled_au;
      bytestream_len = assembled_au_len;
      hinted_codec = assembled_codec;
      input_kind = "rtp-au";
    }
  }
  if (avc_multistream_profile) {
    g_video.adaptive_multistream_enabled = 1;
    adaptive_strip_mode = 1;
  }
  if (g_media_dump_enabled) {
    ard_hp_media_dump_write("au_annexb", g_media_dump_au_index++, bytestream, bytestream_len);
  }
  if (g_video.srtp_unprotect_ok > 0 && (g_video.packets_seen % 120 == 0)) {
    rfbClientLog("ard-hp: SRTP unprotect stats ok=%lu fail=%lu\n",
                 g_video.srtp_unprotect_ok, g_video.srtp_unprotect_fail);
  }

  active_codec = hinted_codec;
  allow_fallback_switch = (hinted_codec == ARD_HP_VIDEO_CODEC_UNKNOWN);
  if (ard_hp_strict_stage2_mode()) {
    allow_fallback_switch = 0;
    if (g_video.codec == ARD_HP_VIDEO_CODEC_UNKNOWN) {
      active_codec = ARD_HP_VIDEO_CODEC_HEVC;
    }
  }
  if (active_codec == ARD_HP_VIDEO_CODEC_UNKNOWN) {
    active_codec = (enum ard_hp_video_codec)g_video.codec;
    if (active_codec == ARD_HP_VIDEO_CODEC_UNKNOWN) {
      active_codec = ARD_HP_VIDEO_CODEC_HEVC;
    }
  }
  if (ard_hp_strict_stage2_mode() &&
      g_video.codec != ARD_HP_VIDEO_CODEC_UNKNOWN &&
      active_codec != (enum ard_hp_video_codec)g_video.codec) {
    active_codec = (enum ard_hp_video_codec)g_video.codec;
  }
  lane_decoders_requested = env_flag_enabled("VNC_ARD_HP_FORCE_LANE_DECODERS");
  use_lane_decoder = (adaptive_strip_mode &&
                      rtp_input &&
                      active_codec == ARD_HP_VIDEO_CODEC_HEVC &&
                      rtp_ssrc != 0u &&
                      lane_decoders_requested);
  ard_hp_update_param_sets_from_annexb(active_codec, bytestream, bytestream_len);
  if (!avc_multistream_profile || use_lane_decoder) {
    if (!ard_hp_prepend_param_sets_if_needed(active_codec, &bytestream, &bytestream_len)) {
      free(bytestream);
      return 1;
    }
  }
  ...
  if (use_lane_decoder) {
    lane_decoder = ard_hp_get_or_create_lane_decoder(rtp_ssrc, active_codec);
    if (!lane_decoder) {
      free(bytestream);
      return 1;
    }
  } else if (!ard_hp_ensure_video_decoder(active_codec)) {
    if (active_codec == ARD_HP_VIDEO_CODEC_HEVC) {
      active_codec = ARD_HP_VIDEO_CODEC_H264;
      if (!ard_hp_ensure_video_decoder(active_codec)) {
        free(bytestream);
        return 1;
      }
    } else {
  ...
  g_video.packets_seen++;
#if defined(__APPLE__) && defined(ARDHPDEBUG_HAS_VIDEOTOOLBOX)
  if (!use_lane_decoder &&
      g_video.backend == ARD_HP_DECODER_BACKEND_VIDEOTOOLBOX &&
      active_codec == ARD_HP_VIDEO_CODEC_H264) {
    (void)allow_fallback_switch;
    (void)ard_hp_vt_decode_h264_annexb(client, bytestream, bytestream_len, &decoded_frames);
    free(bytestream);
    bytestream = NULL;
    if (g_video.packets_seen % 120 == 0) {
      rfbClientLog("ard-hp: video decode backend=videotoolbox codec=%s packets=%lu frames=%lu input=%s decoded_now=%d\n",
                   ard_hp_video_codec_name((enum ard_hp_video_codec)g_video.codec),
                   g_video.packets_seen,
                   g_video.frames_decoded,
                   input_kind,
                   decoded_frames);
...
#endif
  if (use_lane_decoder) {
    ret = ard_hp_decode_with_lane_decoder(client, lane_decoder, bytestream, bytestream_len,
                                          rtp_ssrc, adaptive_strip_mode, &decoded_frames);
    free(bytestream);
    bytestream = NULL;
    if (!ret) return 1;
    if (g_video.packets_seen % 120 == 0) {
      rfbClientLog("ard-hp: video decode codec=%s packets=%lu frames=%lu input=%s decoded_now=%d\n",
                   ard_hp_video_codec_name(active_codec),
                   g_video.packets_seen,
                   g_video.frames_decoded,
                   input_kind,
                   decoded_frames);
  ...
  parse_in = bytestream;
  parse_len = (int)bytestream_len;
  use_parser = (g_video.parser != NULL && active_codec != ARD_HP_VIDEO_CODEC_HEVC);
  ret = 0;
  while (parse_len > 0) {
    uint8_t *pkt_data = (uint8_t *)parse_in;
    int pkt_len = parse_len;
    int consumed = parse_len;
    if (use_parser) {
      consumed = av_parser_parse2(g_video.parser, g_video.codec_ctx,
                                  &pkt_data, &pkt_len,
                                  parse_in, parse_len,
    ...
    if (pkt_len <= 0 || !pkt_data) continue;
    ret = av_new_packet(g_video.packet, pkt_len);
    if (ret < 0) break;
    memcpy(g_video.packet->data, pkt_data, (size_t)pkt_len);
    ret = avcodec_send_packet(g_video.codec_ctx, g_video.packet);
  ...
  bytestream = NULL;
  if (ret < 0 && ret != AVERROR(EAGAIN)) {
    if (allow_fallback_switch) {
      fallback_codec = (active_codec == ARD_HP_VIDEO_CODEC_HEVC) ? ARD_HP_VIDEO_CODEC_H264 : ARD_HP_VIDEO_CODEC_HEVC;
      (void)ard_hp_ensure_video_decoder(fallback_codec);
    }
    return 1;
  }

  for (;;) {
    ret = avcodec_receive_frame(g_video.codec_ctx, g_video.frame);
    if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) break;
    if (ret < 0) {
      ard_hp_log_av_error("avcodec_receive_frame failed", ret);
      break;
    }
    if (ard_hp_push_decoded_frame(client, g_video.frame, rtp_ssrc, adaptive_strip_mode)) {
      decoded_frames++;
    }
...
static void ard_hp_shutdown_video_decoder(void) {
  ...
  ard_hp_shutdown_srtp_contexts();
#if defined(__APPLE__) && defined(ARDHPDEBUG_HAS_VIDEOTOOLBOX)
  ard_hp_vt_release_decoder();
#endif
  for (lane_idx = 0; lane_idx < ARD_HP_ADAPTIVE_LANE_DECODER_MAX; ++lane_idx) {
    ard_hp_release_lane_decoder(&g_video.lane_decoder[lane_idx]);
  }
  g_video.lane_decoder_overflow = 0;
  if (g_video.parser) {
    av_parser_close(g_video.parser);
    g_video.parser = NULL;
  }
  free(g_video.rtp_fu_buf);
  g_video.rtp_fu_buf = NULL;
  g_video.rtp_fu_cap = 0;
  ard_hp_rtp_fu_reset_state(0);
  {
    unsigned long i;
    for (i = 0; i < ARD_HP_ADAPTIVE_FU_CTX_MAX; ++i) {
      ard_hp_adaptive_fu_reset(&g_video.adaptive_fu[i], 0);
      free(g_video.adaptive_fu[i].buf);
      g_video.adaptive_fu[i].buf = NULL;
      g_video.adaptive_fu[i].cap = 0;
      g_video.adaptive_fu[i].len = 0;
      g_video.adaptive_fu[i].ssrc = 0;
    }
    g_video.adaptive_fu_overflow = 0;
  }
  free(g_video.rtp_au_buf);
  g_video.rtp_au_buf = NULL;
  g_video.rtp_au_cap = 0;
  ard_hp_rtp_au_reset_state(0);
  sws_freeContext(g_video.sws);
  g_video.sws = NULL;
  g_video.sws_src_w = 0;
  ...
  g_video.sws_dst_h = 0;
  sws_freeContext(g_video.strip_sws);
  g_video.strip_sws = NULL;
  g_video.strip_sws_src_w = 0;
  g_video.strip_sws_src_h = 0;
  free(g_video.strip_bgra_buf);
  g_video.strip_bgra_buf = NULL;
  g_video.strip_bgra_cap = 0;
  g_video.strip_last_ssrc = 0;
  g_video.strip_frames_composited = 0;
  g_video.adaptive_multistream_enabled = 0;
  memset(g_video.adaptive_lane_ssrc, 0, sizeof(g_video.adaptive_lane_ssrc));
  g_video.adaptive_lane_count = 0;
  g_video.codec = ARD_HP_VIDEO_CODEC_UNKNOWN;
  g_video.backend = ARD_HP_DECODER_BACKEND_NONE;
  g_video.srtp_init_attempted = 0;
  g_video.srtp_contexts_logged = 0;
  g_video.rtp_selected_ssrc = 0;
  g_video.rtp_selected_ssrc_valid = 0;
  g_video.rtp_selected_packets = 0;
  g_video.rtp_dropped_other_ssrc = 0;
  g_video.rtp_selected_frames_at_lock = 0;
...
static void ard_hp_close_socket_fd(int *fd) {
  if (!fd) return;
  if (*fd >= 0) {
    close(*fd);
    *fd = -1;
...
static void ard_hp_media_udp_shutdown(void) {
  ard_hp_close_socket_fd(&g_udp_media.video_sock);
  ard_hp_close_socket_fd(&g_udp_media.audio_sock);
  g_udp_media.enabled = 0;
  g_udp_media.family = 0;
  g_udp_media.video_source_socket = 0;
  g_udp_media.video_socket_substantial_ms = 0;
  g_udp_media.audio_socket_substantial_ms = 0;
  g_udp_media.server_video_port = 0;
  g_udp_media.server_audio_port = 0;
  g_udp_media.local_video_port = 0;
  g_udp_media.local_audio_port = 0;
  g_udp_media.server_ip[0] = '\0';
  g_udp_media.video_packets_rx = 0;
  g_udp_media.audio_packets_rx = 0;
  g_udp_media.video_decode_attempts = 0;
  g_udp_media.video_decode_failures = 0;
  g_udp_media.video_min_len = 0;
  g_udp_media.video_max_len = 0;
  g_udp_media.audio_min_len = 0;
  g_udp_media.audio_max_len = 0;
  g_udp_media.last_stats_log_ms = 0;
  g_udp_media.media_start_ms = 0;
  g_udp_media.last_refresh_retry_ms = 0;
  g_udp_media.last_pli_ms = 0;
...
#if defined(ARDHPDEBUG_HAS_FFMPEG)
  free(g_video.hevc_vps);
  g_video.hevc_vps = NULL;
  g_video.hevc_vps_len = 0;
  free(g_video.hevc_sps);
  g_video.hevc_sps = NULL;
  g_video.hevc_sps_len = 0;
  free(g_video.hevc_pps);
  g_video.hevc_pps = NULL;
  g_video.hevc_pps_len = 0;
#endif
}

static int ard_hp_media_udp_get_peer(rfbClient *client, struct sockaddr_storage *peer_addr,
                                     socklen_t *peer_len, char *ip_out, size_t ip_out_len) {
  socklen_t len;
  int rc;

  if (!client || !peer_addr || !peer_len) return 0;
  memset(peer_addr, 0, sizeof(*peer_addr));
  len = (socklen_t)sizeof(*peer_addr);
  rc = getpeername(client->sock, (struct sockaddr *)peer_addr, &len);
  if (rc != 0) return 0;
  if (peer_addr->ss_family != AF_INET && peer_addr->ss_family != AF_INET6) return 0;
  *peer_len = len;
  if (ip_out && ip_out_len > 0) {
    ip_out[0] = '\0';
    if (peer_addr->ss_family == AF_INET) {
      const struct sockaddr_in *sin = (const struct sockaddr_in *)peer_addr;
      inet_ntop(AF_INET, &sin->sin_addr, ip_out, (socklen_t)ip_out_len);
    } else if (peer_addr->ss_family == AF_INET6) {
      const struct sockaddr_in6 *sin6 = (const struct sockaddr_in6 *)peer_addr;
      inet_ntop(AF_INET6, &sin6->sin6_addr, ip_out, (socklen_t)ip_out_len);
    }
  }
  return 1;
}

static int ard_hp_media_udp_open_connected_socket(int family, const struct sockaddr_storage *peer_addr,
                                                  uint16_t remote_port, uint16_t bind_port,
                                                  int *out_fd, uint16_t *out_local_port) {
  int fd = -1;
  int flags;
  int one = 1;
  ...
  if (!peer_addr || !out_fd || !out_local_port) return 0;
  *out_fd = -1;
  *out_local_port = 0;
  ...
  if (family == AF_INET) {
    ...
    bind_len = (socklen_t)sizeof(*sin);
  } else if (family == AF_INET6) {
    struct sockaddr_in6 *sin6 = (struct sockaddr_in6 *)&bind_addr;
    sin6->sin6_family = AF_INET6;
    sin6->sin6_addr = in6addr_any;
    sin6->sin6_port = htons(bind_port);
    bind_len = (socklen_t)sizeof(*sin6);
  } else {
    close(fd);
    return 0;
  }
  ...
  if (bind(fd, (const struct sockaddr *)&bind_addr, bind_len) == 0) {
    ...
    if (family == AF_INET) {
      ((struct sockaddr_in *)&bind_addr)->sin_port = htons(0);
    } else {
      ((struct sockaddr_in6 *)&bind_addr)->sin6_port = htons(0);
    }
    if (bind(fd, (const struct sockaddr *)&bind_addr, bind_len) == 0) {
      bind_ok = 1;
    }
  ...
  if (family == AF_INET) {
    ((struct sockaddr_in *)&remote_addr)->sin_port = htons(remote_port);
    remote_len = (socklen_t)sizeof(struct sockaddr_in);
  } else {
    ((struct sockaddr_in6 *)&remote_addr)->sin6_port = htons(remote_port);
    remote_len = (socklen_t)sizeof(struct sockaddr_in6);
  }
  if (connect(fd, (const struct sockaddr *)&remote_addr, remote_len) != 0) {
    close(fd);
    return 0;
  }
  ...
  local_len = (socklen_t)sizeof(bind_addr);
  memset(&bind_addr, 0, sizeof(bind_addr));
  if (getsockname(fd, (struct sockaddr *)&bind_addr, &local_len) == 0) {
    if (family == AF_INET) {
      *out_local_port = ntohs(((struct sockaddr_in *)&bind_addr)->sin_port);
    } else {
      *out_local_port = ntohs(((struct sockaddr_in6 *)&bind_addr)->sin6_port);
    }
  }
  *out_fd = fd;
  return 1;
...
static int ard_hp_udp_packet_looks_like_video(const uint8_t *packet, size_t len) {
  const uint8_t *payload = NULL;
  size_t payload_len = 0;
  uint8_t pt = 0;
  uint16_t seq = 0;
  ...
  if (!packet || len < 12u) return 0;
  if (!ard_hp_parse_rtp_payload(packet, len, &payload, &payload_len, &seq, &ssrc, &ts, &marker, &pt)) return 0;
  (void)payload;
  (void)payload_len;
  return pt == 100u;
}

static int ard_hp_media_udp_send_rtcp_packet(int sock_fd, const uint8_t *pkt, size_t pkt_len,
                                             const char *kind);

static int ard_hp_media_udp_send_rtcp_pli_packet(int sock_fd, uint32_t sender_ssrc, uint32_t media_ssrc) {
  uint8_t pkt[12];
  if (sock_fd < 0 || media_ssrc == 0u) return 0;
  pkt[0] = 0x81u; /* V=2, FMT=1 (PLI) */
  pkt[1] = 206u;  /* PSFB */
  pkt[2] = 0x00u;
  pkt[3] = 0x02u; /* 12 bytes / 4 - 1 */
  pkt[4] = (uint8_t)((sender_ssrc >> 24) & 0xffu);
  pkt[5] = (uint8_t)((sender_ssrc >> 16) & 0xffu);
  pkt[6] = (uint8_t)((sender_ssrc >> 8) & 0xffu);
  pkt[7] = (uint8_t)(sender_ssrc & 0xffu);
  pkt[8] = (uint8_t)((media_ssrc >> 24) & 0xffu);
  pkt[9] = (uint8_t)((media_ssrc >> 16) & 0xffu);
  pkt[10] = (uint8_t)((media_ssrc >> 8) & 0xffu);
  pkt[11] = (uint8_t)(media_ssrc & 0xffu);
  return ard_hp_media_udp_send_rtcp_packet(sock_fd, pkt, sizeof(pkt), "PLI");
}

static int ard_hp_media_udp_send_rtcp_fir_packet(int sock_fd, uint32_t sender_ssrc,
                                                 uint32_t media_ssrc, uint8_t seqno) {
  uint8_t pkt[20];
  if (sock_fd < 0 || media_ssrc == 0u) return 0;
  pkt[0] = 0x84u; /* V=2, FMT=4 (FIR) */
  pkt[1] = 206u;  /* PSFB */
  pkt[2] = 0x00u;
  pkt[3] = 0x04u; /* 20 bytes / 4 - 1 */
  pkt[4] = (uint8_t)((sender_ssrc >> 24) & 0xffu);
  pkt[5] = (uint8_t)((sender_ssrc >> 16) & 0xffu);
  pkt[6] = (uint8_t)((sender_ssrc >> 8) & 0xffu);
  pkt[7] = (uint8_t)(sender_ssrc & 0xffu);
  pkt[8] = 0x00u;  /* media source SSRC in FIR header is zero */
  pkt[9] = 0x00u;
  pkt[10] = 0x00u;
  pkt[11] = 0x00u;
  pkt[12] = (uint8_t)((media_ssrc >> 24) & 0xffu); /* FCI media SSRC */
  pkt[13] = (uint8_t)((media_ssrc >> 16) & 0xffu);
  pkt[14] = (uint8_t)((media_ssrc >> 8) & 0xffu);
  pkt[15] = (uint8_t)(media_ssrc & 0xffu);
  pkt[16] = seqno;
  pkt[17] = 0x00u;
  pkt[18] = 0x00u;
  pkt[19] = 0x00u;
  return ard_hp_media_udp_send_rtcp_packet(sock_fd, pkt, sizeof(pkt), "FIR");
}

static int ard_hp_media_udp_send_rtcp_rr_keepalive(int sock_fd, uint32_t sender_ssrc) {
  uint8_t pkt[8];
  if (sock_fd < 0) return 0;
  pkt[0] = 0x80u; /* V=2, RC=0 */
  pkt[1] = 201u;  /* RR */
  pkt[2] = 0x00u;
  pkt[3] = 0x01u; /* 8 bytes / 4 - 1 */
  pkt[4] = (uint8_t)((sender_ssrc >> 24) & 0xffu);
  pkt[5] = (uint8_t)((sender_ssrc >> 16) & 0xffu);
  pkt[6] = (uint8_t)((sender_ssrc >> 8) & 0xffu);
  pkt[7] = (uint8_t)(sender_ssrc & 0xffu);
  return ard_hp_media_udp_send_rtcp_packet(sock_fd, pkt, sizeof(pkt), "RR");
}
...
static int ard_hp_media_udp_send_rtcp_packet(int sock_fd, const uint8_t *pkt, size_t pkt_len,
  ...
  int send_all_keys = 0;
  const char *send_all_keys_env = ard_hp_getenv_compat("VNC_ARD_HP_MEDIA_RTCP_SEND_ALL_KEYS");
#endif
  if (sock_fd < 0 || !pkt || pkt_len == 0) return 0;
  (void)kind;
#if defined(ARDHPDEBUG_HAS_FFMPEG)
  if (send_all_keys_env && env_flag_enabled("VNC_ARD_HP_MEDIA_RTCP_SEND_ALL_KEYS")) {
    send_all_keys = 1;
  }
  ard_hp_init_video_srtp_contexts();
  if (g_video.srtp_tx_ctx_count > 0) {
    if (g_video.srtcp_preferred_tx_ctx_valid &&
        g_video.srtcp_preferred_tx_ctx_idx < g_video.srtp_tx_ctx_count) {
      start_idx = g_video.srtcp_preferred_tx_ctx_idx;
    }
    for (pass = 0; pass < g_video.srtp_tx_ctx_count; ++pass) {
      uint8_t *srtcp = NULL;
      size_t srtcp_len = 0;
      i = (start_idx + pass) % g_video.srtp_tx_ctx_count;
      if (!g_video.srtp_tx_ctx[i]) continue;
      if (!rfbClientARDHPSRTPProtectRTCPPacket(g_video.srtp_tx_ctx[i], pkt, pkt_len, &srtcp, &srtcp_len)) {
        g_video.srtcp_protect_fail++;
        continue;
      }
      sent = send(sock_fd, (const char *)srtcp, srtcp_len, 0);
      free(srtcp);
      if (sent == (ssize_t)srtcp_len) {
        g_video.srtcp_protect_ok++;
        if (!sent_any &&
            (!g_video.srtcp_preferred_tx_ctx_valid || g_video.srtcp_preferred_tx_ctx_idx != i)) {
          g_video.srtcp_preferred_tx_ctx_idx = i;
          g_video.srtcp_preferred_tx_ctx_valid = 1;
        }
        sent_any = 1;
        if (!send_all_keys) return 1;
      }
    }
    if (sent_any) return 1;
  }
#endif
  sent = send(sock_fd, (const char *)pkt, pkt_len, 0);
  return sent == (ssize_t)pkt_len;
}

static unsigned long ard_hp_media_udp_send_keyframe_requests(void) {
  int sockets[2];
  unsigned long i;
  unsigned long socket_count = 0;
  unsigned long sent = 0;
#if defined(ARDHPDEBUG_HAS_FFMPEG)
  unsigned long si;
#endif

  if (!g_udp_media.enabled) return 0;
  if (g_udp_media.video_sock >= 0) sockets[socket_count++] = g_udp_media.video_sock;
  if (g_udp_media.audio_sock >= 0 &&
      (socket_count == 0 || g_udp_media.audio_sock != sockets[0])) {
    sockets[socket_count++] = g_udp_media.audio_sock;
  }
  if (socket_count == 0) return 0;

  if (g_udp_media.rtcp_sender_ssrc == 0u) g_udp_media.rtcp_sender_ssrc = 0x4d434c49u; /* MCLI */
  for (i = 0; i < socket_count; ++i) {
    if (ard_hp_media_udp_send_rtcp_rr_keepalive(sockets[i], g_udp_media.rtcp_sender_ssrc)) {
      g_udp_media.rr_keepalives_sent++;
    }
  }
#if defined(ARDHPDEBUG_HAS_FFMPEG)
  for (si = 0; si < socket_count; ++si) {
    int sock_fd = sockets[si];
    for (i = 0; i < g_video.rtp_ssrc_track_count; ++i) {
      uint32_t ssrc = g_video.rtp_ssrc_tracks[i].ssrc;
      if (ssrc == 0u) continue;
      if (ard_hp_media_udp_send_rtcp_pli_packet(sock_fd, g_udp_media.rtcp_sender_ssrc, ssrc)) sent++;
      if (ard_hp_media_udp_send_rtcp_fir_packet(sock_fd, g_udp_media.rtcp_sender_ssrc,
                                                ssrc, g_udp_media.rtcp_fir_seq)) {
        sent++;
      }
      g_udp_media.rtcp_fir_seq++;
    }
  }
#endif
  return sent;
...
static int ard_hp_media_udp_maybe_start(rfbClient *client) {
  ...
  uint16_t stage1_port;
  uint16_t video_port;
  uint16_t audio_port;
  uint16_t video_bind_port;
  uint16_t audio_bind_port;
  const char *udp_mode_env;
  int video_fd = -1;
  int audio_fd = -1;
  uint16_t local_video = 0;
  uint16_t local_audio = 0;

  if (!client) return 1;
  if (!g_runtime.ard_hp_mode || !env_flag_enabled("VNC_ARD_HP_ADAPTIVE_MODE")) return 1;
  if (!g_hp.media_stream_active) return 1;
  if (g_hp.media_stream_message_type < 2) {
    if (g_hp.media_stream_message_type != 1 ||
        (g_hp.media_stage1_udp_port == 0 && g_hp.media_stream_next_udp_port == 0 &&
         client->serverPort <= 0)) {
      return 1;
    }
    rfbClientLog("ard-hp: starting UDP media sockets from stage1 metadata to avoid missing initial keyframes\n");
  }
  if (g_udp_media.enabled) return 1;

  udp_mode_env = ard_hp_getenv_compat("VNC_ARD_HP_MEDIA_UDP");
  if (udp_mode_env && !env_flag_enabled("VNC_ARD_HP_MEDIA_UDP")) return 1;

  stage1_port = g_hp.media_stage1_udp_port;
  if (stage1_port == 0) {
    if (g_hp.media_stream_base_udp_port != 0) {
      stage1_port = g_hp.media_stream_base_udp_port;
    } else if (g_hp.media_stream_next_udp_port >= 1024u) {
      /* Some stage2 headers carry non-port small fields; avoid treating those as UDP base ports. */
      stage1_port = g_hp.media_stream_next_udp_port;
    } else {
      stage1_port = (uint16_t)client->serverPort;
    }
  }

  video_port = env_u16_or_default("VNC_ARD_HP_MEDIA_UDP_VIDEO_PORT", stage1_port);
  audio_port = env_u16_or_default("VNC_ARD_HP_MEDIA_UDP_AUDIO_PORT", (uint16_t)(video_port + 1u));
  video_bind_port = env_u16_or_default("VNC_ARD_HP_MEDIA_UDP_VIDEO_BIND_PORT", video_port);
  audio_bind_port = env_u16_or_default("VNC_ARD_HP_MEDIA_UDP_AUDIO_BIND_PORT", audio_port);

  if (!ard_hp_media_udp_get_peer(client, &peer_addr, &peer_len, g_udp_media.server_ip,
                                 sizeof(g_udp_media.server_ip))) {
    rfbClientErr("ard-hp: udp media start failed: cannot resolve peer address\n");
    return 0;
  }
  (void)peer_len;

  if (!ard_hp_media_udp_open_connected_socket((int)peer_addr.ss_family, &peer_addr, video_port, video_bind_port,
                                              &video_fd, &local_video)) {
    rfbClientErr("ard-hp: udp media start failed: cannot open video socket remote=%u bind=%u\n",
                 (unsigned)video_port, (unsigned)video_bind_port);
    return 0;
  }
  if (!ard_hp_media_udp_open_connected_socket((int)peer_addr.ss_family, &peer_addr, audio_port, audio_bind_port,
                                              &audio_fd, &local_audio)) {
    rfbClientLog("ard-hp: udp media audio socket unavailable remote=%u bind=%u; continuing video-only\n",
                 (unsigned)audio_port, (unsigned)audio_bind_port);
  }

  g_udp_media.video_sock = video_fd;
  g_udp_media.audio_sock = audio_fd;
  g_udp_media.enabled = 1;
  g_udp_media.family = (int)peer_addr.ss_family;
  g_udp_media.server_video_port = video_port;
  g_udp_media.server_audio_port = audio_port;
  g_udp_media.local_video_port = local_video;
  g_udp_media.local_audio_port = local_audio;
  g_udp_media.media_start_ms = monotonic_ms();
  g_hp.media_udp_started = 1;
  rfbClientLog("ard-hp: udp media receiver started server=%s video=%u(local=%u) audio=%u(local=%u)\n",
               g_udp_media.server_ip[0] ? g_udp_media.server_ip : "(unknown)",
               (unsigned)video_port, (unsigned)local_video,
               (unsigned)audio_port, (unsigned)local_audio);
  {
    uint16_t request_w = ard_hp_request_width(client);
    uint16_t request_h = ard_hp_request_height(client);
    if (request_w == 0 || request_h == 0) {
      request_w = client->width > 0 ? (uint16_t)client->width : 0;
      request_h = client->height > 0 ? (uint16_t)client->height : 0;
    }
    if (request_w > 0 && request_h > 0) {
      (void)rfbClientARDHPSendAutoFramebufferUpdate(client, request_w, request_h);
      (void)ard_hp_request_full_refresh_now(client, request_w, request_h, "udp-media-start");
    }
  }
  return 1;
}
...
static int ard_hp_media_udp_pump(rfbClient *client) {
  uint8_t packet[8192];
  int loops = 0;
  int video_max_reads = (int)env_u16_or_default("VNC_ARD_HP_MEDIA_UDP_VIDEO_MAX_READS", 2048u);
  int audio_max_reads = (int)env_u16_or_default("VNC_ARD_HP_MEDIA_UDP_AUDIO_MAX_READS", 2048u);
  size_t min_video_rtp_len = (size_t)env_u16_or_default("VNC_ARD_HP_MEDIA_MIN_RTP_LEN", 64u);

  if (!client) return 1;
  if (!g_udp_media.enabled || g_udp_media.video_sock < 0) return 1;

  while (loops < video_max_reads) {
    ssize_t nread;
    int looks_like_video = 0;
    int decode_this = 0;
    uint8_t pt = 0;
    nread = recv(g_udp_media.video_sock, (char *)packet, sizeof(packet), 0);
    if (nread < 0) {
      if (errno == EAGAIN || errno == EWOULDBLOCK) break;
      if (errno == ECONNREFUSED) break;
      rfbClientErr("ard-hp: udp video recv failed (%d: %s)\n", errno, strerror(errno));
      break;
    }
    if (nread == 0) break;
    g_udp_media.video_packets_rx++;
    if (g_udp_media.video_min_len == 0 || (size_t)nread < g_udp_media.video_min_len) g_udp_media.video_min_len = (size_t)nread;
    if ((size_t)nread > g_udp_media.video_max_len) g_udp_media.video_max_len = (size_t)nread;
      looks_like_video = ard_hp_udp_packet_looks_like_video(packet, (size_t)nread);
      if (looks_like_video && (size_t)nread >= 12u) {
    ...
    if (decode_this) {
      ...
      if (!ard_hp_decode_video_media_packet(client, 0x3ea, packet, (size_t)nread)) {
        g_udp_media.video_decode_failures++;
        return 0;
      }
    }
    loops++;
  ...
  if (g_udp_media.audio_sock >= 0) {
    int audio_loops = 0;
    while (audio_loops < audio_max_reads) {
      ssize_t nread;
      int looks_like_video = 0;
      int decode_this = 0;
      uint8_t pt = 0;
      nread = recv(g_udp_media.audio_sock, (char *)packet, sizeof(packet), 0);
      if (nread < 0) {
        if (errno == EAGAIN || errno == EWOULDBLOCK || errno == ECONNREFUSED) break;
        rfbClientErr("ard-hp: udp audio recv failed (%d: %s)\n", errno, strerror(errno));
        break;
      }
      if (nread == 0) break;
      g_udp_media.audio_packets_rx++;
      if (g_udp_media.audio_min_len == 0 || (size_t)nread < g_udp_media.audio_min_len) g_udp_media.audio_min_len = (size_t)nread;
      if ((size_t)nread > g_udp_media.audio_max_len) g_udp_media.audio_max_len = (size_t)nread;
      looks_like_video = ard_hp_udp_packet_looks_like_video(packet, (size_t)nread);
      if (looks_like_video && (size_t)nread >= 12u) {
        pt = (uint8_t)(packet[1] & 0x7fu);
        if (pt == 100u && (size_t)nread >= 300u) {
  ...
  if (g_udp_media.video_packets_rx > 0 || g_udp_media.audio_packets_rx > 0) {
    ...
    unsigned long srtcp_fail = 0;
    unsigned long fu_ok = 0;
    unsigned long fu_drop = 0;
    unsigned long au_ok = 0;
    unsigned long au_drop = 0;
    uint32_t selected_ssrc = 0;
    unsigned long selected_packets = 0;
    unsigned long dropped_other_ssrc = 0;
    unsigned long ssrc_tracks = 0;
    unsigned long ssrc_overflow = 0;
    unsigned long video_decoded_packets = 0;
    unsigned long video_decoded_frames = 0;
    unsigned long strip_frames = 0;
    unsigned long strip_lanes = 0;
    uint32_t strip_ssrc = 0;
#if defined(ARDHPDEBUG_HAS_FFMPEG)
    srtp_ok = g_video.srtp_unprotect_ok;
    srtp_fail = g_video.srtp_unprotect_fail;
    srtcp_ok = g_video.srtcp_protect_ok;
    srtcp_fail = g_video.srtcp_protect_fail;
    fu_ok = g_video.rtp_fu_completed;
    fu_drop = g_video.rtp_fu_dropped;
    au_ok = g_video.rtp_au_completed;
    au_drop = g_video.rtp_au_dropped;
    selected_ssrc = g_video.rtp_selected_ssrc_valid ? g_video.rtp_selected_ssrc : 0;
    selected_packets = g_video.rtp_selected_packets;
    dropped_other_ssrc = g_video.rtp_dropped_other_ssrc;
    ssrc_tracks = g_video.rtp_ssrc_track_count;
    ssrc_overflow = g_video.rtp_ssrc_overflow_packets;
    video_decoded_packets = g_video.packets_seen;
    video_decoded_frames = g_video.frames_decoded;
    strip_frames = g_video.strip_frames_composited;
    strip_lanes = g_video.adaptive_lane_count;
    ...
    if (now_ms > 0) {
      if (video_decoded_frames > g_udp_media.last_decoded_frames_observed) {
        ...
        g_udp_media.last_decode_progress_ms = now_ms;
      } else if (g_udp_media.last_decode_progress_ms == 0) {
        g_udp_media.last_decode_progress_ms = now_ms;
      }
    }
    if (now_ms > 0 &&
        (g_udp_media.last_rr_keepalive_ms == 0 || now_ms - g_udp_media.last_rr_keepalive_ms >= 1000)) {
      if (g_udp_media.rtcp_sender_ssrc == 0u) g_udp_media.rtcp_sender_ssrc = 0x4d434c49u; /* MCLI */
      if (g_udp_media.video_sock >= 0 &&
          ard_hp_media_udp_send_rtcp_rr_keepalive(g_udp_media.video_sock, g_udp_media.rtcp_sender_ssrc)) {
        g_udp_media.rr_keepalives_sent++;
      }
      if (g_udp_media.audio_sock >= 0 &&
          g_udp_media.audio_sock != g_udp_media.video_sock &&
          ard_hp_media_udp_send_rtcp_rr_keepalive(g_udp_media.audio_sock, g_udp_media.rtcp_sender_ssrc)) {
        g_udp_media.rr_keepalives_sent++;
      }
      g_udp_media.last_rr_keepalive_ms = now_ms;
    }
    if (fu_drop > g_udp_media.last_fu_drop_observed) {
    ...
    if (now_ms > 0 &&
        g_udp_media.video_decode_attempts > 0 &&
        g_udp_media.refresh_retries < 6u &&
        (strip_frames == 0 || strip_lanes < 4u) &&
        (g_udp_media.last_refresh_retry_ms == 0 || now_ms - g_udp_media.last_refresh_retry_ms >= 2500)) {
      uint16_t request_w = ard_hp_request_width(client);
      uint16_t request_h = ard_hp_request_height(client);
      if (request_w == 0 || request_h == 0) {
        request_w = client->width > 0 ? (uint16_t)client->width : 0;
      ...
      if (request_w > 0 && request_h > 0) {
        (void)rfbClientARDHPSendAutoFramebufferUpdate(client, request_w, request_h);
        if (ard_hp_request_full_refresh_now(client, request_w, request_h, "udp-media-retry")) {
          g_udp_media.refresh_retries++;
          g_udp_media.last_refresh_retry_ms = now_ms;
          rfbClientLog("ard-hp: udp media retry refresh %lu/%d (%ux%u)\n",
                       g_udp_media.refresh_retries, 6, request_w, request_h);
        }
      }
    }
    if (env_flag_enabled("VNC_ARD_HP_MEDIA_PLI_LANE_DISCOVERY") &&
        strip_lanes < 4u &&
        now_ms > 0 &&
        g_udp_media.media_start_ms > 0 &&
        now_ms - g_udp_media.media_start_ms >= 3000) {
      keyframe_recovery_needed = 1;
      keyframe_recovery_reason = "lane-discovery";
    }
    if (!keyframe_recovery_needed &&
        now_ms > 0 &&
        g_udp_media.video_decode_attempts >= 256u &&
        g_udp_media.last_decode_progress_ms > 0 &&
        now_ms - g_udp_media.last_decode_progress_ms >= 1000) {
      keyframe_recovery_needed = 1;
      keyframe_recovery_reason = "decode-stall";
      if (env_flag_enabled("VNC_ARD_HP_ADAPTIVE_SINGLE_SSRC_FALLBACK") &&
          !g_video.rtp_selected_ssrc_valid &&
          strip_ssrc != 0u) {
      ...
      if (stall_flush_enabled &&
        ...
        g_udp_media.last_decoder_flush_ms = now_ms;
        g_udp_media.decoder_flushes++;
        rfbClientLog("ard-hp: flushed video decoder state for stall recovery count=%lu\n",
                     g_udp_media.decoder_flushes);
      }
      if (env_flag_enabled("VNC_ARD_HP_STALL_REINIT") &&
          g_runtime.ard_hp_mode &&
          env_flag_enabled("VNC_ARD_HP_ADAPTIVE_MODE") &&
          g_hp.media_stream_active &&
          g_hp.media_stream_message_type >= 2 &&
          now_ms - g_udp_media.last_decode_progress_ms >= 3000 &&
          (g_udp_media.last_media_reinit_ms == 0 ||
           now_ms - g_udp_media.last_media_reinit_ms >= 5000)) {
        g_hp.adaptive_media_options_sent = 0;
        if (ard_hp_send_adaptive_media_options_after_stage1(client)) {
          g_udp_media.last_media_reinit_ms = now_ms;
          g_udp_media.media_reinit_attempts++;
          rfbClientLog("ard-hp: resent adaptive media options for stall recovery attempt=%lu\n",
                       g_udp_media.media_reinit_attempts);
        } else {
          g_hp.adaptive_media_options_sent = 1;
        }
      }
    }
    if (now_ms > 0 &&
      ...
      if (pli_sent > 0) {
        ...
        g_udp_media.pli_requests_sent += pli_sent;
        rfbClientLog("ard-hp: sent RTCP PLI keyframe requests count=%lu total=%lu reason=%s\n",
    ...
    if (stall_full_refresh_enabled &&
      ...
      if (request_w > 0 && request_h > 0) {
        (void)rfbClientARDHPSendAutoFramebufferUpdate(client, request_w, request_h);
        if (ard_hp_request_full_refresh_now(client, request_w, request_h, "decode-stall-recovery")) {
          g_udp_media.last_refresh_retry_ms = now_ms;
          rfbClientLog("ard-hp: sent full refresh request for decode-stall recovery (%ux%u)\n",
    ...
    if (now_ms > 0 && (g_udp_media.last_stats_log_ms == 0 || now_ms - g_udp_media.last_stats_log_ms >= 2000)) {
      ...
      rfbClientLog("ard-hp: udp media stats video_rx=%lu(len=%lu..%lu) audio_rx=%lu(len=%lu..%lu) decode_attempts=%lu decode_failures=%lu srtp_ok=%lu srtp_fail=%lu srtcp_ok=%lu srtcp_fail=%lu rr_sent=%lu fu_ok=%lu fu_drop=%lu au_ok=%lu au_drop=%lu dec_pkts=%lu dec_frames=%lu strip_frames=%lu strip_lanes=%lu strip_last_ssrc=0x%08x ssrc_sel=0x%08x sel_pkts=%lu ssrc_drop=%lu ssrc_tracks=%lu ssrc_overflow=%lu src_socket=%d failovers=%lu\n",
                   ...
                   g_udp_media.audio_packets_rx,
                   (unsigned long)g_udp_media.audio_min_len,
                   (unsigned long)g_udp_media.audio_max_len,
                   g_udp_media.video_decode_attempts,
                   g_udp_media.video_decode_failures,
                   srtp_ok,
                   srtp_fail,
                   ...
                   au_ok,
                   au_drop,
                   video_decoded_packets,
                   video_decoded_frames,
                   strip_frames,
                   strip_lanes,
                   (unsigned)strip_ssrc,
                   (unsigned)selected_ssrc,
                   selected_packets,
                   dropped_other_ssrc,
                   ssrc_tracks,
                   ssrc_overflow,
...
static int ard_hp_maybe_pump_media_input(rfbClient *client) {
  ...
  if (!client) return 1;
  if (!ard_hp_media_udp_pump(client)) return 0;
...
static double ard_hp_runtime_scale_factor(rfbClient *client) {
  if (g_runtime.ard_hp_mode && env_flag_enabled("VNC_ARD_HP_ADAPTIVE_MODE") && client) {
    ...
    if (backing_w == 1 && backing_h == 1) return 1.0;
...
static int ard_hp_send_post_rekey_setup(rfbClient *client) {
  ...
  if (env_flag_enabled("VNC_ARD_HP_DISABLE_AUTO_PASTEBOARD")) {
    send_auto_pasteboard = 0;
  }
  if (send_auto_pasteboard) {
  ...
  if (!ard_hp_resize_framebuffer_if_needed(client, region_w, region_h)) return 0;
  if (!SendCurrentPixelFormat(client)) return 0;
  if (!SendFramebufferUpdateRequest(client, 0, 0, region_w, region_h, FALSE)) return 0;
  g_hp.initial_full_refresh_done = 0;
  g_hp.initial_full_refresh_retries = adaptive_mode ? 0 : 3;
  ...
  if (adaptive_mode) {
    rfbClientLog("ard-hp: adaptive mode defers MediaStreamOptions 0x1c until media-init stage1 (0x3f2)\n");
  }
...
static int ard_hp_send_adaptive_media_options_after_stage1(rfbClient *client) {
  ...
  char *generated_media_options_hex = NULL;
  const char *delayed_setenc_env = NULL;
  int delayed_setenc = 1;
  int strict_stage2 = 0;
  int send_setenc_immediately = 0;

  if (!client) return 1;
  if (!g_runtime.ard_hp_mode || !env_flag_enabled("VNC_ARD_HP_ADAPTIVE_MODE")) return 1;
  if (g_hp.adaptive_media_options_sent) return 1;

  region_w = ard_hp_request_width(client);
  region_h = ard_hp_request_height(client);

  rfbClientLog("ard-hp: adaptive stage1 follow-up step=SetPixelFormat\n");
  if (!SendCurrentPixelFormat(client)) return 0;
  rfbClientLog("ard-hp: adaptive stage1 follow-up step=FramebufferUpdateRequest %ux%u\n",
               (unsigned)region_w, (unsigned)region_h);
  if (!SendFramebufferUpdateRequest(client, 0, 0, region_w, region_h, FALSE)) return 0;
  rfbClientLog("ard-hp: adaptive stage1 follow-up step=AutoFrameBufferUpdate %ux%u\n",
               (unsigned)region_w, (unsigned)region_h);
  if (!rfbClientARDHPSendAutoFramebufferUpdate(client, region_w, region_h)) return 0;

  media_options_hex = getenv("VNC_ARD_HP_MEDIA_STREAM_OPTIONS_HEX");
  if (media_options_hex && *media_options_hex) {
    rfbClientLog("ard-hp: adaptive stage1 follow-up step=MediaStreamOptionsHex\n");
    if (!rfbClientARDHPSendMediaStreamOptionsHex(client, media_options_hex)) return 0;
    ard_hp_set_last_media_options_hex(media_options_hex);
    ard_hp_try_add_srtp_keys_from_media_options_hex(media_options_hex);
  } else {
    if (rfbClientARDHPBuildAdaptiveMediaStreamOptionsHex(&generated_media_options_hex) &&
        generated_media_options_hex && *generated_media_options_hex) {
      rfbClientLog("ard-hp: adaptive stage1 follow-up step=AdaptiveMediaStreamOptionsHexGenerated\n");
      if (!rfbClientARDHPSendMediaStreamOptionsHex(client, generated_media_options_hex)) {
        free(generated_media_options_hex);
        return 0;
      }
      ard_hp_set_last_media_options_hex(generated_media_options_hex);
      ard_hp_try_add_srtp_keys_from_media_options_hex(generated_media_options_hex);
      free(generated_media_options_hex);
      generated_media_options_hex = NULL;
    } else {
      rfbClientLog("ard-hp: adaptive stage1 follow-up step=AdaptiveMediaStreamOptions\n");
      if (!rfbClientARDHPSendAdaptiveMediaStreamOptions(client)) return 0;
    }
  }
  g_hp.adaptive_media_options_sent = 1;
  strict_stage2 = ard_hp_strict_stage2_mode();
  delayed_setenc_env = ard_hp_getenv_compat("VNC_ARD_HP_ADAPTIVE_DELAYED_SET_ENCODINGS");
  delayed_setenc = delayed_setenc_env ? env_flag_enabled("VNC_ARD_HP_ADAPTIVE_DELAYED_SET_ENCODINGS") : 1;
  if (strict_stage2) {
    g_hp.adaptive_delayed_setenc_pending = 0;
    g_hp.adaptive_delayed_setenc_sent = 1;
    send_setenc_immediately = 0;
    rfbClientLog("ard-hp: strict-stage2 enabled; deferring follow-up SetEncodings until after stage2\n");
  } else {
    g_hp.adaptive_delayed_setenc_pending = delayed_setenc ? 1 : 0;
    g_hp.adaptive_delayed_setenc_sent = delayed_setenc ? 0 : 1;
    send_setenc_immediately = delayed_setenc ? 0 : 1;
  }
  if (send_setenc_immediately) {
    rfbClientLog("ard-hp: adaptive stage1 follow-up sending SetEncodings immediately\n");
    if (!rfbClientARDHPSendPostAuthEncodings(client)) return 0;
    g_hp.adaptive_followup_encodings_sent = 1;
  }
  g_hp.adaptive_post_1c_server_rects = 0;
  rfbClientLog("ard-hp: adaptive stage1 follow-up sent (SetPixelFormat + full refresh + AutoFBU + MediaStreamOptions 0x1c)\n");
  return 1;
}
...
static rfbBool handle_hp_probe_encoding(rfbClient *client, rfbFramebufferUpdateRectHeader *rect) {
  if (client) client->suppressCurrentFramebufferUpdateCallbacks = TRUE;
  uint8_t buf[36];
  uint8_t hdr[2];
  ...
  size_t total_len;
  int is_media_stage1 = 0;
  int is_media_stage2 = 0;
  ...
  if ((uint32_t)rect->encoding == 0x44f) {
    ...
    ard_hp_media_udp_shutdown();
    g_hp.post_rekey_ready = 1;
    g_hp.post_rekey_phase = 0;
    g_hp.media_udp_started = 0;
    g_hp.media_stage1_udp_port = 0;
    g_hp.adaptive_media_options_sent = 0;
    g_hp.adaptive_delayed_setenc_pending = 0;
    g_hp.adaptive_delayed_setenc_sent = 0;
    g_hp.adaptive_post_1c_server_rects = 0;
    g_hp.adaptive_followup_encodings_sent = 0;
    if (adaptive_mode) {
  ...
  if (!ard_hp_strict_stage2_mode() &&
      g_hp.adaptive_delayed_setenc_pending &&
      !g_hp.adaptive_delayed_setenc_sent &&
      (uint32_t)rect->encoding != 0x3f2 &&
      (uint32_t)rect->encoding != 0x3f3 &&
      (uint32_t)rect->encoding != 0x3ea) {
    g_hp.adaptive_post_1c_server_rects++;
    if (g_hp.adaptive_post_1c_server_rects >= 2) {
      rfbClientLog("ard-hp: adaptive delayed follow-up sending SetEncodings after %u post-0x1c server rects\n",
                   g_hp.adaptive_post_1c_server_rects);
      if (!rfbClientARDHPSendPostAuthEncodings(client)) {
        free(payload);
        return FALSE;
      }
      g_hp.adaptive_delayed_setenc_sent = 1;
  ...
  if ((uint32_t)rect->encoding == 0x3f2) {
    if (total_len >= 6) {
      is_media_stage1 = (total_len >= 36 &&
                         read_be_u16(payload + 0) == 36 &&
                         read_be_u16(payload + 2) == 1 &&
                         read_be_u16(payload + 4) == 1);
      is_media_stage2 = (read_be_u16(payload + 2) == 2 &&
                         read_be_u16(payload + 4) == 2);
    }
    ...
    if (is_media_stage1 && g_hp.media_stream_next_udp_port != 0) {
      g_hp.media_stage1_udp_port = g_hp.media_stream_next_udp_port;
    }
    ...
    if (g_runtime.ard_hp_mode && env_flag_enabled("VNC_ARD_HP_ADAPTIVE_MODE")) {
      if (is_media_stage1) {
        if (!g_hp.media_udp_started) {
          if (!ard_hp_media_udp_maybe_start(client)) {
            free(payload);
            return FALSE;
          }
        }
      ...
      } else if (is_media_stage2) {
        if (!ard_hp_media_udp_maybe_start(client)) {
          free(payload);
          return FALSE;
        }
        if (!g_hp.adaptive_followup_encodings_sent) {
          if (ard_hp_strict_stage2_mode()) {
            rfbClientLog("ard-hp: strict-stage2 enabled; skipping stage2 follow-up SetEncodings\n");
            g_hp.adaptive_followup_encodings_sent = 1;
          } else {
            rfbClientLog("ard-hp: adaptive stage2 received; sending follow-up SetEncodings\n");
            if (!rfbClientARDHPSendPostAuthEncodings(client)) {
              free(payload);
              return FALSE;
            }
      ...
      } else if (g_hp.media_stream_message_type == 1) {
        if (!g_hp.adaptive_media_options_sent) {
          rfbClientLog("ard-hp: observed non-stage1 MediaStreamMessage1 payload (len=%u version=%u type=%u), sending adaptive stage1 follow-up once\n",
                       (unsigned)payload_len,
                       (unsigned)g_hp.media_stream_version,
                       (unsigned)g_hp.media_stream_message_type);
          if (!ard_hp_send_adaptive_media_options_after_stage1(client)) {
          ...
          rfbClientLog("ard-hp: observed non-stage1 MediaStreamMessage1 payload (len=%u version=%u type=%u), skipping 0x1c resend\n",
                       (unsigned)payload_len,
                       (unsigned)g_hp.media_stream_version,
                       (unsigned)g_hp.media_stream_message_type);
        }
      }
      if (g_hp.media_stream_message_type >= 2 && !g_hp.media_udp_started) {
        if (!ard_hp_media_udp_maybe_start(client)) {
          free(payload);
          return FALSE;
        }
  ...
  if ((uint32_t)rect->encoding == 0x3f3 || (uint32_t)rect->encoding == 0x3ea) {
    ard_hp_log_media_codec_probe((uint32_t)rect->encoding, payload, total_len);
    ...
    if (!ard_hp_decode_video_media_packet(client, (uint32_t)rect->encoding, payload, total_len)) {
      free(payload);
      return FALSE;
    }
    free(payload);
  ...
  if ((uint32_t)rect->encoding == 0x451 && total_len >= 12) {
    ...
    }
    {
      ...
      int startup_full_refresh_deferred = 0;
      ...
      if ((g_hp.dynamic_request_in_flight || g_hp.dynamic_request_timed_out) &&
        ...
        if (pending_target_w != 0 &&
          ...
          g_hp.dynamic_refresh_queued_for_request = 1;
                 ...
                 !(first_display_layout && startup_full_refresh_deferred)) {
        g_hp.pending_refresh_w = request_w;
        g_hp.pending_refresh_h = request_h;
        g_hp.initial_full_refresh_retries = 0;
        g_hp.dynamic_refresh_retries = 0;
  ...
  if ((uint32_t)rect->encoding == 0x455 && env_flag_enabled("VNC_ARD_HP_SEND_KEYBOARD_SOURCE_SHARE")) {
    const char *fallback_source = "com.apple.keylayout.US";
    const char *mode = getenv("VNC_ARD_HP_KEYBOARD_SOURCE_SHARE_MODE");
    char *source = NULL;
    uint16_t source_len = 0;
    const uint8_t *source_ptr = NULL;
    int shared = 1;

    if (mode && *mode && strcasecmp(mode, "disable") == 0) shared = 0;

    if (total_len >= 10) {
      source_len = read_be_u16(payload + 8);
      if ((size_t)source_len + 10u <= total_len) source_ptr = payload + 10;
    }
    if (source_ptr && source_len > 0) {
      source = (char *)calloc(1, (size_t)source_len + 1u);
      if (!source) {
        free(payload);
        return FALSE;
      }
      memcpy(source, source_ptr, source_len);
      source[source_len] = '\0';
      while (source_len > 0 && source[source_len - 1] == '\0') source[--source_len] = '\0';
    }
    if (!ard_hp_send_keyboard_source_share(client, source ? source : fallback_source, shared)) {
      free(source);
      free(payload);
      return FALSE;
    }
...
int main(int argc, char **argv) {
  ...
  if (env_flag_enabled("VNC_ARD_HP_ADAPTIVE_MODE")) {
    client->width = 1;
    client->height = 1;
  }
...
wait_for_server:
    ...
    if (!ard_hp_maybe_pump_media_input(client)) break;
#if defined(ARDHPDEBUG_HAS_SDL)
